...
//
// SecretGram user-facing text follows Telegram's selected interface language,
// never the device locale. English is the canonical source and universal
// fallback. Persisted SecretGram data stores semantic values, not translations.
public enum SecretGramStringKey: String, CaseIterable {
    case settingsTitle
    case basicFunctions
    case ghostMode
    case messages
...
public struct SecretGramStrings {
    ...
    public func text(_ key: SecretGramStringKey) -> String {
    ...
    private static let english: [SecretGramStringKey: String] = [
        .settingsTitle: "SecretGram",
        ...
        .portableReplyHint: "The reply is materialized only after Send. Media is kept only in SecretGram's internal cache: up to 1 GB for 30 days; if bytes are unavailable, a text fallback is used.",
        ...
        .appearancePlaceholder: "SecretGram appearance settings will be added here.",
        ...
        .profileEffectDisabledHint: "When the main effect is disabled, SecretGram creates no additional profile views, observers, or image/palette pipeline. New values apply the next time the profile opens.",
        .other: "Other",
        .interface: "Interface",
        .messageSeconds: "Message Seconds",
        .hideMyPhone: "Hide My Phone Number",
        .showRamUnderClock: "Show RAM Under Clock",
        .hidePhoneHint: "Your phone number is hidden only locally in SecretGram. Profile editing and number changing remain available.",
        ...
        .importArchive: "Import SecretGram Archive",
        .exportArchive: "Export SecretGram Archive",
        ...
        .community: "SecretGram Community",
    ...
    private static let russian: [SecretGramStringKey: String] = [
        .settingsTitle: "SecretGram",
        ...
        .portableReplyHint: "Ответ материализуется только после Send. Медиа хранится только во внутреннем кэше SecretGram: до 1 ГБ, 30 дней; если bytes недоступны, используется текстовый fallback.",
        ...
        .appearancePlaceholder: "Настройки оформления SecretGram будут добавляться в этот раздел.",
        ...
        .profileEffectDisabledHint: "Когда главный эффект выключен, SecretGram не создаёт дополнительные profile views, observers или image/palette pipeline. Новые значения применяются при следующем открытии профиля.",
        .other: "Прочее",
        .interface: "Интерфейс",
        .messageSeconds: "Секунды в сообщениях",
        .hideMyPhone: "Скрывать мой номер",
        .showRamUnderClock: "Показывать RAM под часами",
        .hidePhoneHint: "Номер скрывается только локально в интерфейсе SecretGram. Экран изменения профиля и смены номера остаётся доступен.",
        ...
        .importArchive: "Импорт архива SecretGram",
        .exportArchive: "Экспорт архива SecretGram",
        ...
        .community: "Сообщество SecretGram",
...
public extension PresentationStrings {
    var secretgram: SecretGramStrings {
        return SecretGramStrings(
...
public extension SecretGramStrings {
...
public extension SecretGramStrings {
    ...
    var build124DiagnosticsSummary: String {
        if self.languageCode == "ru" {
            return "Диагностика и исследовательские инструменты SecretGram"
        } else {
            return "SecretGram diagnostics and research tools"
        }
    }

    var build124AboutSummary: String {
        return SecretGramReleaseIdentity.aboutText
    }
...
// same SecretGramStrings owner as the rest of SecretGram. Persisted diagnostic
...
public extension SecretGramStrings {
    ...
    var botAlreadyAdded: String { self.botIsRussian ? "Этот бот уже добавлен в SecretGram." : "This bot is already added to SecretGram." }

    var botLogoutTitle: String { self.botIsRussian ? "Выйти из аккаунта бота?" : "Log out of the bot account?" }
    var botLogoutText: String { self.botIsRussian ? "Аккаунт будет удалён только из SecretGram. Сам бот и его токен в BotFather не удаляются." : "The account will be removed only from SecretGram. The bot itself and its BotFather token are not deleted." }
...
public extension SecretGramStrings {
    ...
    var secretgramVersion: String { self.languageCode == "ru" ? "Версия SecretGram" : "SecretGram Version" }
    ...
    var anonymousAnalyticsDescription: String {
        if self.languageCode == "ru" {
            return "Помогайте улучшать SecretGram, отправляя анонимную статистику использования: версию SecretGram, версию iOS и регион устройства. Аккаунты Telegram, сообщения, имена пользователей и номера телефонов никогда не собираются."
        } else {
            return "Help improve SecretGram by sharing anonymous usage statistics such as SecretGram version, iOS version and device region. Telegram accounts, messages, usernames and phone numbers are never collected."
        }
    }
}
...
public enum SecretGramReleaseIdentity {
    public static let displayVersion = "1.0.0"
    public static let technicalVersion = "1.0.0"
    public static let build = "138"
    public static let telegramBase = "12.9.2"

    public static var aboutText: String {
        return "SecretGram Version \(displayVersion)\nBuild \(build)\nTelegram Base \(telegramBase)"
    }
}
