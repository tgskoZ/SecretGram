        ...
        |> mapToSignal { [weak self] peers, requiresStars -> Signal<ShareState, ShareControllerError> in
            ...
            case let .messages(messages):
                for peerId in peerIds {
                    ...
                    let ghostBasePostShareScheduledEnabled = ((SecretGramHotSettings.object(forKey: "secretgram.GhostMode.ScheduledSend") as? Bool) ?? false)
                    ...
                    if ghostBasePostShareScheduledEnabled {
                        ...
                        UserDefaults.standard.set(
                            UserDefaults.standard.integer(
                                forKey: "secretgram.SH1.ShareScheduledIntercept.Count"
                            ) + 1,
                            forKey: "secretgram.SH1.ShareScheduledIntercept.Count"
                        )
                        ...
                        UserDefaults.standard.set(
                            "\(peerId)",
                            forKey: "secretgram.SH1.LastSharePeerId"
                        )

                        UserDefaults.standard.set(
                            messagesToEnqueue.count,
                            forKey: "secretgram.SH1.LastShareMessageCount"
                        )

                        UserDefaults.standard.set(
                            Int(ghostBasePostShareScheduleTime),
                            forKey: "secretgram.SH1.LastShareScheduleTime"
                        )
                    }
