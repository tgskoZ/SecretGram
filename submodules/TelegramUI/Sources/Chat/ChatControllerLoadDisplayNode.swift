...
private func secretgramBuild135VisibleUnreadCount(
    account: Account,
    peerId: PeerId?,
    stockCount: Int
) -> Signal<Int, NoError> {
    ...
    return account.postbox.transaction { transaction -> Int in
        guard let chatPeer = transaction.getPeer(peerId),
              SecretGramBlockedReactionPolicy.isGroupChat(chatPeer),
              SecretGramBlockedReactionPolicy.hideBlockedMessages(accountPeerId: account.peerId) else {
            return stockCount
        }
    ...
    func setupChatHistoryNode(historyNode: ChatHistoryListNodeImpl) {
        ...
        do {
            ...
            if let subject = self.subject, case .scheduledMessages = subject {
                ...
                let throttledUnreadCountSignal = combineLatest(
                ...
                |> mapToSignal { [weak self] count, _ -> Signal<Int, NoError> in
                    ...
                    return secretgramBuild135VisibleUnreadCount(
