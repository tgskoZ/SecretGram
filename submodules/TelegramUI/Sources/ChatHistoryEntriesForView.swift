...
func chatHistoryEntriesForView(
    ...
    loop: for entry in view.entries {
        ...
        if skipViewOnceMedia, let minAutoremoveOrClearTimeout = message.minAutoremoveOrClearTimeout {
            let ghostBaseOT2KeepViewOnceVisible = (((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.Enabled") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.OneTimeSave") as? Bool) ?? false) && message.id.peerId.namespace != Namespaces.Peer.SecretChat)
            if minAutoremoveOrClearTimeout <= 60 {
                if ghostBaseOT2KeepViewOnceVisible {
                    UserDefaults.standard.set(UserDefaults.standard.integer(forKey: "secretgram.OT2.ViewOnceVisibleKeep.Count") + 1, forKey: "secretgram.OT2.ViewOnceVisibleKeep.Count")
                    UserDefaults.standard.set("\(message.id)", forKey: "secretgram.OT2.LastViewOnceVisibleId")
                } else {
                    continue loop
                }
            }
        ...
        let ghostBaseShowDeleted = ((SecretGramHotSettings.object(forKey: "secretgram.Messages.ShowDeleted") as? Bool) ?? true)
