...
private func secretgramRequiresPortableForward(_ message: Message) -> Bool {
...
private func secretgramPortableForwardBaseMessage(
    _ message: Message,
    hideAuthor: Bool,
    threadId: Int64?,
...
private func secretgramProtectedResourceData(
...
private func secretgramPortableForwardMessage(
    _ message: Message,
    hideAuthor: Bool,
    threadId: Int64?,
    context: AccountContext
) -> Signal<EnqueueMessage, NoError> {
    guard secretgramRequiresPortableForward(message), let media = message.media.first else {
        let reference = message.media.first.map { AnyMediaReference.standalone(media: $0) }
        return .single(secretgramPortableForwardBaseMessage(
            message,
    ...
    if let file = media as? TelegramMediaFile {
        ...
        return secretgramProtectedResourceData(
            context: context,
            message: message,
        ...
        |> map { data -> EnqueueMessage in
            ...
            return secretgramPortableForwardBaseMessage(
                message,
                hideAuthor: hideAuthor,
                threadId: threadId,
            ...
            controller.multiplePeersSelected = { [weak self, weak controller] peers, peerMap, messageText, mode, forwardOptions, _ in
                ...
                |> deliverOnMainQueue).start(next: { [weak self, weak controller] sendPaidMessageStars, renderedPeers in
                    ...
                    let proceed = { [weak self, weak controller] in
                        ...
                        var secretgramPortableMessagesSignal: Signal<[EnqueueMessage], NoError>?
                        if messageText.string.count > 0 {
                        ...
                        if canUsePortableCopy && (hideAuthor || messages.contains(where: { secretgramRequiresPortableForward($0) })) {
                            let secretgramPrefixMessages = result
                            secretgramPortableMessagesSignal = combineLatest(messages.map { message in
                                secretgramPortableForwardMessage(
                                    message,
                                    hideAuthor: hideAuthor,
                            ...
                            |> map { portableMessages in
                                secretgramPrefixMessages + portableMessages
                            }
                        } else {
                        ...
                        let commit: ([EnqueueMessage]) -> Void = { result in
                            ...
                            |> deliverOnMainQueue).startStandalone(next: { targetPeersShouldDivert in
                                ...
                                for (peer, shouldDivert) in targetPeersShouldDivert {
                                    ...
                                    let ghostBaseSH1ScheduledSendEnabled = ((SecretGramHotSettings.object(forKey: "secretgram.GhostMode.ScheduledSend") as? Bool) ?? false)
                                    if ghostBaseSH1ScheduledSendEnabled && !shouldDivert {
                                        ...
                                        UserDefaults.standard.set(UserDefaults.standard.integer(forKey: "secretgram.SH1.ShareScheduledIntercept.Count") + 1, forKey: "secretgram.SH1.ShareScheduledIntercept.Count")
                                        UserDefaults.standard.set("\(peer.id)", forKey: "secretgram.SH1.LastSharePeerId")
                                        UserDefaults.standard.set(peerMessages.count, forKey: "secretgram.SH1.LastShareMessageCount")
                                        UserDefaults.standard.set(Int(ghostBaseSH1ScheduleTime), forKey: "secretgram.SH1.LastShareScheduleTime")
                                    }
                        ...
                        if let secretgramPortableMessagesSignal {
                            let _ = (secretgramPortableMessagesSignal
