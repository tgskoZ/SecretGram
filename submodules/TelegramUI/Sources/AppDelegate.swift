...
private func secretgramResolvedApplicationGroupIdentifier(
    fallback: String
) -> String {
...
// SECRETGRAM_LEGACY_NAMESPACE_BEGIN
//
//
...
//
// Canonical namespace remains secretgram.*.
// Legacy values are copied, never deleted.
// Existing secretgram.* values always win.
//
private enum SecretGramLegacyDefaultsMigration {
    private static let canonicalPrefix = "secretgram."

    private static let migrationMarker =
        "secretgram.runtime.namespaceMigration.v1"
    ...
    private static func migrateDomain(
        ...
        print(
            "[SecretGram] migrated \\(migratedCount) legacy defaults in \\(domainName)"
        )
    }
...
// SECRETGRAM_LEGACY_NAMESPACE_END



// SecretGram does not send analytics to the upstream project.

final class SecretGramMemorySampler {
    static let shared = SecretGramMemorySampler()

    private let queue = DispatchQueue(label: "secretgram.memory.sampler", qos: .utility)
...
@objc(AppDelegate) class AppDelegate: UIResponder, UIApplicationDelegate, PKPushRegistryDelegate, UNUserNotificationCenterDelegate, URLSessionDelegate, URLSessionTaskDelegate {
    ...
    private func urlSession(identifier: String) -> URLSession {
        ...
        let appGroupName = secretgramResolvedApplicationGroupIdentifier(fallback: "group.\(baseAppBundleId)")
BuildConfig.secretgramRecordExtensionDiagnostic(
    process: "app",
    stage: "profile",
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]? = nil) -> Bool {
        SecretGramLegacyDefaultsMigration.run()
        ...
        let applicationBindings = TelegramApplicationBindings(isMainApp: true, appBundleId: baseAppBundleId, appBuildType: buildConfig.isAppStoreBuild ? .public : .internal, containerPath: appGroupUrl.path, appSpecificScheme: buildConfig.appSpecificUrlScheme, openUrl: { url in
           ...
           clearMessageNotifications: { ids in
            ...
            if #available(iOS 10.3, *) {
                var icons = [
                    PresentationAppIcon(name: "SecretGramSteelReveal", imageName: "SecretGramSteelReveal"),
                    PresentationAppIcon(name: "SecretGramSteelSolid", imageName: "SecretGramSteelSolid"),
                    PresentationAppIcon(name: "SecretGramRustReveal", imageName: "SecretGramRustReveal"),
                    PresentationAppIcon(name: "SecretGramRustSolid", imageName: "SecretGramRustSolid"),
                    PresentationAppIcon(name: "SecretGramInkReveal", imageName: "SecretGramInkReveal"),
                    PresentationAppIcon(name: "SecretGramInkSolid", imageName: "SecretGramInkSolid"),
                    PresentationAppIcon(name: "SecretGramOliveReveal", imageName: "SecretGramOliveReveal"),
                    PresentationAppIcon(name: "SecretGramOliveSolid", imageName: "SecretGramOliveSolid"),
                    PresentationAppIcon(name: "SecretGramGlassReveal", imageName: "SecretGramGlassRevealPreview", isDefault: true),
                    PresentationAppIcon(name: "SecretGramGlassSolid", imageName: "SecretGramGlassSolidPreview"),
...
BuildConfig.secretgramRecordExtensionDiagnostic(
    ...
    detail: "account metadata owner initialized"
        ...
        let secretgramMemoryTimer = Foundation.Timer.scheduledTimer(withTimeInterval: 0.5, repeats: true, block: { _ in
            SecretGramMemorySampler.shared.sample(consumer: "appDelegate") { value in
        ...
        secretgramMemoryTimer.tolerance = 0.25
