...
public final class ChatControllerImpl: TelegramBaseController, ChatController, GalleryHiddenMediaTarget, UIDropInteractionDelegate {
    ...
    public init(
        ...
        let controllerInteraction = ChatControllerInteraction(openMessage: { [weak self] message, params in
            ...
            let openChatMessageParams = OpenChatMessageParams(
                ...
                copyProtected: !(((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.AllowScreenshots") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.AllowScreenRecording") as? Bool) ?? true)) && (self.presentationInterfaceState.copyProtectionEnabled || self.presentationInterfaceState.myCopyProtectionEnabled),
            ...
            let ghostBaseAllowCapture = (((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.AllowScreenshots") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.AllowScreenRecording") as? Bool) ?? true))
    ...
    override public func viewDidAppear(_ animated: Bool) {
        ...
        if case .standard(.default) = self.mode, let chatPeerId = self.chatLocation.peerId {
            ...
            Queue.concurrentDefaultQueue().async { [weak self] in
                ...
                let watermarkStore = SecretGramVisitWatermarkStore(rootURL: rootURL)
                let previousSequence = watermarkStore.previousSequence(
                    accountPeerId: accountPeerId,
                    chatPeerId: chatPeerIdValue
                )
                guard let records = try? SecretGramCaptureRecorder.readyIndexRecords(
                    accountPeerId: accountPeerId,
                    chatPeerId: chatPeerIdValue,
                ...
                Queue.mainQueue().async { [weak self] in
                    ...
                    let text = self.presentationData.strings.secretgram.changesSinceLastOpening(
                    ...
                    self.present(UndoOverlayController(
                        ...
                        content: .info(
                            ...
                            text: text,
                            timeout: 6.0,
                            customUndoText: self.presentationData.strings.secretgram.timeMachine
                        ),
                        elevatedLayout: false,
                        position: .top,
                        action: { [weak self] action in
                            guard case .undo = action, let self else { return false }
                            let controller = secretgramTimeMachineController(
                                context: self.context,
                                chatPeerId: chatPeerIdValue,
                                initialQuery: "",
    ...
    func transformEnqueueMessages(_ messages: [EnqueueMessage], silentPosting: Bool, scheduleTime: Int32? = nil, repeatPeriod: Int32? = nil, postpone: Bool = false) -> [EnqueueMessage] {
        let ghostBaseScheduledSendEnabled = (SecretGramHotSettings.object(forKey: "secretgram.GhostMode.ScheduledSend") as? Bool) ?? false
        let ghostBaseScheduleBaseTime = Int32(Date().timeIntervalSince1970) + 12

        let ghostBaseSendTextStyle = (SecretGramHotSettings.object(forKey: "secretgram.Messages.SendTextStyle") as? String) ?? "normal"
    ...
    private func commitEnqueueMediaMessages(
        ...
        |> deliverOnMainQueue).startStrict(next: { [weak self] items in
            ...
            |> deliverOnMainQueue).startStandalone(next: { shouldDivert in
                ...
                let ghostBaseRuntimeScheduledSend = ((SecretGramHotSettings.object(forKey: "secretgram.GhostMode.ScheduledSend") as? Bool) ?? false) && scheduleTime == nil
    ...
    func enqueueChatContextResult(_ results: ChatContextResultCollection, _ result: ChatContextResult, hideVia: Bool = false, closeMediaInput: Bool = false, silentPosting: Bool = false, scheduleTime: Int32? = nil, resetTextInputState: Bool = true, postpone: Bool = false) {
        ...
        let sendMessage: (Int32?, Bool) -> Void = { [weak self] scheduleTime, silentPosting in
            ...
            let ghostBaseScheduledSend = (
                SecretGramHotSettings.object(forKey: "secretgram.GhostMode.ScheduledSend") as? Bool
            ) ?? false
    ...
    private func secretgramNumericMentionPeerId(_ source: String) -> Int64? {
    ...
    func openPeerMention(_ name: String, navigation: ChatControllerInteractionNavigateToPeer = .default, sourceMessageId: MessageId? = nil, progress: Promise<Bool>? = nil) {
        let _ = self.presentVoiceMessageDiscardAlert(action: {
            if let idValue = self.secretgramNumericMentionPeerId(name) {
