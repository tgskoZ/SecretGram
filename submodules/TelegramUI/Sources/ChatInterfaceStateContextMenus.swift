...
func contextMenuForChatPresentationInterfaceState(chatPresentationInterfaceState: ChatPresentationInterfaceState, context: AccountContext, messages: [EngineRawMessage], controllerInteraction: ChatControllerInteraction?, selectAll: Bool, interfaceInteraction: ChatPanelInterfaceInteraction?, readStats: MessageReadStats? = nil, messageNode: ChatMessageItemView? = nil) -> Signal<ContextController.Items, NoError> {
    ...
    |> map { args -> ContextController.Items in
        ...
        let ghostBaseShowEditHistory = ((SecretGramHotSettings.object(forKey: "secretgram.Messages.ShowEditHistory") as? Bool) ?? true)
        ...
        if ghostBaseShowEditHistory && !ghostBaseEditHistoryVersions.isEmpty {
            actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.secretgram.messageHistory, icon: { theme in
                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Copy"), color: theme.actionSheet.primaryTextColor)
            }, action: { _, f in
                ...
                controller.title = chatPresentationInterfaceState.strings.secretgram.messageHistory
                controllerInteraction.navigationController()?.pushViewController(controller)
                f(.default)
            })))
        }

        // for focused single-message actions, with the canonical secretgram.*
        // key retained only as migration/default fallback.
        let legacyForwardWithoutAuthorKey = "secretgram.Messages.ForwardWithoutAuthor"
        let scopedForwardWithoutAuthorKey = "secretgram.account.\(context.account.peerId.toInt64()).setting.\(legacyForwardWithoutAuthorKey)"
        ...
        let secretgramForwardWithoutAuthorTargets = selectAll ? messages : [message]
        if ghostBaseForwardWithoutAuthor,
           secretgramForwardWithoutAuthorTargets.allSatisfy({ message in
            ...
            actions.append(.action(ContextMenuActionItem(
                text: chatPresentationInterfaceState.strings.secretgram.forwardWithoutAuthor,
                icon: { theme in
                    generateTintedImage(
                        image: UIImage(
                            bundleImageName:
        ...
        if !messageText.isEmpty || richMessageMarkdown != nil || (resourceAvailable && isImage) || diceEmoji != nil {
            if !isExpired {
                if !isPoll {
                    let ghostBaseProtectedChatCopy = (((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.Enabled") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.ChatCopy") as? Bool) ?? true) && !message.containsSecretMedia)
        ...
        let ghostBaseProtectedChatSave = (((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.Enabled") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.ChatSave") as? Bool) ?? true))

        let ghostBaseTimedMediaSave = (((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.Enabled") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.OneTimeSave") as? Bool) ?? false) && message.id.peerId.namespace != Namespaces.Peer.SecretChat && message.paidContent == nil && message.minAutoremoveOrClearTimeout != nil)
        ...
        if data.messageActions.options.contains(.forward) {
            let ghostBaseProtectedChatForwardMenu = (((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.Enabled") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.ChatForward") as? Bool) ?? true) && !message.containsSecretMedia && message.paidContent == nil)
                ...
                if id.namespace == Namespaces.Message.EphemeralLocal {
                    ...
                    if let channel = peer as? TelegramChannel {
                        ...
                        if !message.containsSecretMedia && !isAction && !isShareProtected {
                            if message.id.peerId.namespace != Namespaces.Peer.SecretChat && (!message.isCopyProtected() || (((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.Enabled") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.ChatForward") as? Bool) ?? true))) {
                        ...
                        if message.id.peerId.namespace != Namespaces.Peer.SecretChat && !message.containsSecretMedia {
                            if !isAction && (!message.isCopyProtected() || (((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.Enabled") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.ChatForward") as? Bool) ?? true))) && !isShareProtected {
                                if !(message.flags.isSending || message.flags.contains(.Failed)) {
                                    optionsMap[id]!.insert(.forward)
                                }
                            }
                        ...
                        if !isScheduled && message.id.peerId.namespace != Namespaces.Peer.SecretChat && !message.containsSecretMedia && !isAction && !message.id.peerId.isReplies && (!message.isCopyProtected() || (((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.Enabled") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.ChatForward") as? Bool) ?? true))) && !isShareProtected {
                            if !(message.flags.isSending || message.flags.contains(.Failed)) {
                                optionsMap[id]!.insert(.forward)
                            }
                        }
...
private func ghostBaseEditHistoryKey(_ id: MessageId) -> String {
    return "secretgram.EditHistory.\(id.peerId).\(id.namespace).\(id.id)"
}
