...
private func secretgramBuild135ProjectAccountSettings(accountPeerId: Int64) {
    defer {
    ...
    let targetPrefix = "secretgram.account.\(accountPeerId).setting."
    let runtimeKeys: [String] = [
        "secretgram.Profile.Enabled",
        "secretgram.Profile.ShowIds",
        "secretgram.Profile.ShowDCs",
        "secretgram.Profile.ShowRegistration",
        "secretgram.Glass.Enabled",
        "secretgram.ProfileBlur.Avatar",
        "secretgram.ProfileBlur.Animated",
        "secretgram.ProfileBlur.Tint",
        "secretgram.ProfileBlur.Reduced",
        "secretgram.GhostMode.ReadMessages",
        "secretgram.GhostMode.TypingActions",
        "secretgram.GhostMode.HideRecording",
        "secretgram.GhostMode.HideUploading",
        "secretgram.GhostMode.HideStickerActivity",
        "secretgram.GhostMode.HideGameActivity",
        "secretgram.GhostMode.HideEmojiActivity",
        "secretgram.GhostMode.Presence",
        "secretgram.GhostMode.ScheduledSend",
        "secretgram.Messages.SaveDeleted",
        "secretgram.Messages.ShowDeleted",
        "secretgram.Messages.SaveEditHistory",
        "secretgram.Messages.ShowEditHistory",
        "secretgram.Messages.HideBlockedMessages",
        "secretgram.Messages.HideBlockedReactions",
        "secretgram.Messages.SendTextStyle",
        "secretgram.Messages.DeletedPortableReplies",
        "secretgram.Messages.PreserveDeletedMedia",
        "secretgram.Appearance.ShowRamUnderClock",
        "secretgram.Appearance.MessageSeconds",
        "secretgram.Appearance.HideOwnPhone",
        "secretgram.ProtectedContent.Enabled",
        "secretgram.ProtectedContent.GalleryShare",
        "secretgram.ProtectedContent.GallerySave",
        "secretgram.ProtectedContent.GalleryCopy",
        "secretgram.ProtectedContent.ChatSave",
        "secretgram.ProtectedContent.ChatCopy",
        "secretgram.ProtectedContent.ChatForward",
        "secretgram.ProtectedContent.AllowScreenshots",
        ...
        "secretgram.ProtectedContent.OneTimeSave",
        "secretgram.Stories.Save",
    ...
    let scheduledSendKey = "secretgram.GhostMode.ScheduledSend"
...
public final class SharedAccountContextImpl: SharedAccountContext {
        ...
        |> deliverOnMainQueue).start(next: { primaryId, records, authRecord in
            ...
            |> deliverOnMainQueue).start(next: { mappedAddedAccounts, authAccount in
                ...
                if primary !== self.activeAccountsValue!.primary {
                    ...
                    if let primary {
                        secretgramBuild135ProjectAccountSettings(accountPeerId: primary.account.peerId.toInt64())
                    }
                }
    ...
    public func switchToAccount(id: AccountRecordId, fromSettingsController settingsController: ViewController? = nil, withChatListController chatListController: ViewController? = nil) {
        ...
        if let target = self.activeAccountsValue?.accounts.first(where: { $0.0 == id })?.1 {
            secretgramBuild135ProjectAccountSettings(accountPeerId: target.account.peerId.toInt64())
        }
