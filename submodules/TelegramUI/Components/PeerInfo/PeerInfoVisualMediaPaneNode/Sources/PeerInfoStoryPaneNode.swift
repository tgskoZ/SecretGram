...
private final class SparseItemGridBindingImpl: SparseItemGridBinding {
    ...
    func createLayer(item: SparseItemGrid.Item) -> SparseItemGridLayer? {
        if let item = item as? VisualMediaItem, item.story.isForwardingDisabled && !(((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.AllowScreenshots") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.AllowScreenRecording") as? Bool) ?? true)) {
            let layer = ItemLayer()
