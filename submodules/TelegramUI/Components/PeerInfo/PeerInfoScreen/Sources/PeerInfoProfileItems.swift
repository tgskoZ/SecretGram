...
struct GhostBaseProfileMetricsSettings {
    ...
    static func load() -> GhostBaseProfileMetricsSettings {
        ...
        return GhostBaseProfileMetricsSettings(
            enabled: defaults.object(forKey: "secretgram.Profile.Enabled") as? Bool ?? true,
            showIds: defaults.object(forKey: "secretgram.Profile.ShowIds") as? Bool ?? true,
            showDCs: defaults.object(forKey: "secretgram.Profile.ShowDCs") as? Bool ?? true,
            showRegistration: defaults.object(forKey: "secretgram.Profile.ShowRegistration") as? Bool ?? true,
            hideOwnPhone: defaults.object(forKey: "secretgram.Appearance.HideOwnPhone") as? Bool ?? false
        )
    }
}
...
private enum GhostBaseProfileAuxiliaryStoreV11G {
    static let queue = DispatchQueue(
        label: "secretgram.ProfileAuxiliaryStore.V11G",
        qos: .utility
    )
