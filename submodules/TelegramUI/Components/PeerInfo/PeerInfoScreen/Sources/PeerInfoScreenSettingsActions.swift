                ...
                if user.botInfo != nil {
                    self.controller?.present(
                        textAlertController(
                            context: self.context,
                            updatedPresentationData: self.controller?.updatedPresentationData,
                            title: self.presentationData.strings.secretgram.botLogoutTitle,
                            text: self.presentationData.strings.secretgram.botLogoutText,
                            actions: [
                                TextAlertAction(
                                    type: .genericAction,
                                ...
                                TextAlertAction(
                                    type: .defaultAction,
                                    title: self.presentationData.strings.secretgram.botLogoutAction,
                                    action: { [weak self] in
                                        guard let self else {
                                            return
                                        }
                                        UserDefaults.standard.removeObject(
                                            forKey: "secretgram.BotAccount.\(user.id.toInt64())"
                                        )
