...
final class PeerInfoListPaneNode: ASDisplayNode, PeerInfoPaneNode {
    ...
    private let secretgramLinksReadabilityEnabled: Bool
    ...
    init(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil, chatControllerInteraction: ChatControllerInteraction, peerId: EnginePeer.Id, chatLocation: ChatLocation, chatLocationContextHolder: Atomic<ChatLocationContextHolder?>, tagMask: EngineMessage.Tags, ghostBaseGlassEnabled: Bool = false) {
        ...
        self.secretgramLinksReadabilityEnabled = tagMask == .webPage
    ...
    func update(size: CGSize, topInset: CGFloat, sideInset: CGFloat, bottomInset: CGFloat, deviceMetrics: DeviceMetrics, visibleHeight: CGFloat, isScrollingLockedAtTop: Bool, expandProgress: CGFloat, navigationHeight: CGFloat, presentationData: PresentationData, synchronous: Bool, transition: ContainedViewLayoutTransition) {
        ...
        if self.ghostBaseGlassEnabled && !self.secretgramLinksReadabilityEnabled {
        ...
        } else if !self.secretgramLinksReadabilityEnabled {
            self.glassBackgroundView.isHidden = true
            transition.updateFrame(view: self.glassBackgroundView, frame: self.secretgramLinksReadabilityEnabled ? .zero : self.glassBackgroundView.frame)
        }
        ...
        if self.secretgramLinksReadabilityEnabled {
