    ...
    public static func asyncLayout(_ maybeNode: ChatMessageReplyInfoNode?) -> (_ arguments: Arguments) -> (CGSize, (CGSize, Bool, ListViewItemUpdateAnimation) -> ChatMessageReplyInfoNode) {
        ...
        return { arguments in
            ...
            return (size, { realSize, attemptSynchronous, animation in
                ...
                if let message = arguments.message {
                    node.imageNode?.captureProtected = !(((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.AllowScreenshots") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.AllowScreenRecording") as? Bool) ?? true)) && (arguments.associatedData.isCopyProtectionEnabled || message.isCopyProtected())
                }
