...
private func contentNodeMessagesAndClassesForItem(_ item: ChatMessageItem) -> ([(Message, AnyClass, ChatMessageEntryAttributes, BubbleItemAttributes)], Bool, Bool) {
    ...
    if !reactionsAreInline && !hideAllAdditionalInfo, let reactionsAttribute = secretgramVisibleMessageReactions(accountPeerId: item.context.account.peerId, message: firstMessage), !reactionsAttribute.reactions.isEmpty {
...
public class ChatMessageBubbleItemNode: ChatMessageItemView, ChatMessagePreviewItemNode {
    ...
    private static func beginLayout(
        ...
        if incoming, case let .customChatContents(contents) = item.associatedData.subject, case .hashTagSearch = contents.kind, !contents.ghostBaseSuppressSearchJump {
            ...
            let ghostBaseProtectedChatForward = (((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.Enabled") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.ChatForward") as? Bool) ?? true) && !item.message.containsSecretMedia && item.message.paidContent == nil)
        ...
        if needReactions || forceReactionsOutside {
            bubbleReactions = secretgramVisibleMessageReactions(accountPeerId: item.context.account.peerId, message: item.message) ?? ReactionsMessageAttribute(canViewList: false, isTags: false, reactions: [], recentPeers: [], topPeers: [])
        } else {
