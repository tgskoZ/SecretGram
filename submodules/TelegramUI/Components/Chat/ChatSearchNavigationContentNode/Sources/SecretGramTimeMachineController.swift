...
private struct SecretGramTimeMachineUIState: Equatable {
    var kinds: Set<SecretGramEventKind>
    var senderPeerId: Int64?
}

private struct SecretGramTimeMachinePageState: Equatable {
    var events: [SecretGramCanonicalEvent]
    var hasMore: Bool
}

private final class SecretGramTimeMachineUIArguments {
    let toggleKind: (SecretGramEventKind) -> Void
    let selectSender: () -> Void
    let selectEvent: (SecretGramCanonicalEvent) -> Void
    let loadMore: () -> Void
    init(
        toggleKind: @escaping (SecretGramEventKind) -> Void,
        selectSender: @escaping () -> Void,
        selectEvent: @escaping (SecretGramCanonicalEvent) -> Void,
...
private enum SecretGramTimeMachineUIEntry: ItemListNodeEntry {
    case header(Int32, String)
    case summary(Int32, Int32, String, String)
    case filter(Int32, Int32, String, String, SecretGramEventKind?)
    case result(Int32, Int32, String, String, SecretGramCanonicalEvent)
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        let arguments = arguments as! SecretGramTimeMachineUIArguments
        switch self {
...
private func secretgramTimeMachineDateText(_ timestampMs: Int64, dateTimeFormat: PresentationDateTimeFormat) -> String {
...
private func secretgramTimeMachineRootURL() -> URL {
    return FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
        .appendingPathComponent("SecretGram", isDirectory: true)
}

private func secretgramEventKindTitle(_ kind: SecretGramEventKind, strings: SecretGramStrings) -> String {
    switch kind {
...
private func secretgramDiffText(_ event: SecretGramCanonicalEvent) -> String {
    ...
    return SecretGramTextDiff.diff(old: old, new: new).map { operation in
        switch operation {
...
public func secretgramTimeMachineController(
    ...
    let eventsValue = Atomic<[SecretGramCanonicalEvent]>(value: [])
    let eventsPromise = ValuePromise(
        SecretGramTimeMachinePageState(events: [], hasMore: true),
        ignoreRepeated: true
    )
    let pageLock = NSLock()
    var beforeSequence: Int64?
    var beforeEventId: SecretGramEventId?
    var hasMore = true
    var isLoading = false
    let loadNextPage: () -> Void = {
        ...
        Queue.concurrentDefaultQueue().async {
            let store = SecretGramJSONLEventStore(rootURL: secretgramTimeMachineRootURL())
            ...
            pageLock.unlock()
            eventsPromise.set(SecretGramTimeMachinePageState(events: events, hasMore: pageHasMore))
        }
    }
    loadNextPage()
    let initial = SecretGramTimeMachineUIState(
    ...
    let arguments = SecretGramTimeMachineUIArguments(toggleKind: { kind in
        ...
        if let namespace = event.messageNamespace, let id = event.messageId {
            ...
            let localDetail = secretgramDiffText(event)
            controller?.present(textAlertController(
                context: context,
                title: presentationData.strings.secretgram.timeMachine,
                text: localDetail,
    ...
    |> map { presentationData, state, page -> (ItemListControllerState, (ItemListNodeState, Any)) in
        ...
        var entries: [SecretGramTimeMachineUIEntry] = [
        ...
        for (index, event) in results.enumerated() {
            ...
            let date = secretgramTimeMachineDateText(event.observedAtMs, dateTimeFormat: presentationData.dateTimeFormat)
            let kind = secretgramEventKindTitle(event.kind, strings: strings)
