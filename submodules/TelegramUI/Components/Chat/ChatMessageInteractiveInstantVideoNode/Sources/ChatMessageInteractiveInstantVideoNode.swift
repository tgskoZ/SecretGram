                ...
                if let strongSelf = self {
                    ...
                    if let telegramFile = updatedFile {
                        if updatedMedia {
                            ...
                            }), content: NativeVideoContent(id: .message(item.message.stableId, telegramFile.fileId), userLocation: .peer(item.message.id.peerId), fileReference: .message(message: MessageReference(item.message), media: telegramFile), streamVideo: streamVideo ? .conservative : .none, enableSound: false, fetchAutomatically: false, isAudioVideoMessage: true, captureProtected: (isViewOnceMessage && !(((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.OneTimeScreenshots") as? Bool) ?? false) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.OneTimeScreenRecording") as? Bool) ?? false) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.Enabled") as? Bool) ?? true) && item.message.id.peerId.namespace != Namespaces.Peer.SecretChat)) || (!(((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.AllowScreenshots") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.AllowScreenRecording") as? Bool) ?? true)) && (item.associatedData.isCopyProtectionEnabled || item.message.isCopyProtected())), storeAfterDownload: nil), priority: item.associatedData.isStandalone ? .overlay : .embedded, autoplay: item.context.sharedContext.energyUsageSettings.autoplayVideo && !isViewOnceMessage)
    ...
    private func transcribe() {
        ...
        if shouldBeginTranscription {
            if self.transcribeDisposable == nil {
                ...
                Logger.shared.log("secretgram.Transcription2", "round video still requires Official server transcription")
