        ...
        private func performOtherMoreAction(sourceView: UIView, gesture: ContextGesture?) {
            ...
            |> take(1)).startStandalone(next: { [weak self] result, translationSettings, baseRate in
                ...
                if case let .user(user) = component.slice.peer, let botInfo = user.botInfo {
                    ...
                    } else if !component.slice.item.storyItem.isForwardingDisabled || ((UserDefaults.standard.object(forKey: "secretgram.Stories.Save") as? Bool) ?? false) {
                        let saveText: String = component.strings.Story_Context_SaveToGallery
                        items.append(.action(ContextMenuActionItem(text: saveText, icon: { theme in
                            return generateTintedImage(image: UIImage(bundleImageName: (accountUser.isPremium || ((UserDefaults.standard.object(forKey: "secretgram.Stories.Save") as? Bool) ?? false)) ? "Chat/Context Menu/Download" : "Chat/Context Menu/DownloadLocked"), color: theme.contextMenu.primaryColor)
                        }, action: { [weak self] _, a in
                            a(.default)
                            ...
                            if accountUser.isPremium || ((UserDefaults.standard.object(forKey: "secretgram.Stories.Save") as? Bool) ?? false) {
                                self.requestSave()
                            } else {
