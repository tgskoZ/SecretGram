...
public struct SecretGramSettingsSnapshot: Codable, Equatable {
...
public enum SecretGramArchiveComponent: String, Codable, CaseIterable {
...
public struct SecretGramArchivePayloadDescriptor: Codable, Equatable {
    public let component: SecretGramArchiveComponent
...
public struct SecretGramArchiveAccountManifest: Codable, Equatable {
    public let accountPeerId: Int64
    public let payloads: [SecretGramArchivePayloadDescriptor]

    public init(
        accountPeerId: Int64,
        payloads: [SecretGramArchivePayloadDescriptor]
    ) {
        self.accountPeerId = accountPeerId
...
public struct SecretGramArchiveManifestV2: Codable, Equatable {
    public let schemaVersion: Int
    public let createdAtMs: Int64
    public let accounts: [SecretGramArchiveAccountManifest]

    public init(createdAtMs: Int64, accounts: [SecretGramArchiveAccountManifest]) {
...
public enum SecretGramArchiveValidationError: Error, Equatable {
...
public enum SecretGramArchiveV2 {
    ...
    public static func validateRelativePath(_ value: String) throws {
        ...
        guard !value.isEmpty, !value.hasPrefix("/"),
            ...
            throw SecretGramArchiveValidationError.unsafePath(value)
        }
    }
    ...
    public static func validateExtractedPayloads(
        ...
        guard manifest.schemaVersion == self.schemaVersion else {
            throw SecretGramArchiveValidationError.unsupportedVersion(manifest.schemaVersion)
        }
        let descriptors = manifest.accounts.flatMap(\.payloads)
        guard descriptors.count <= self.maximumPayloadCount else {
            throw SecretGramArchiveValidationError.sizeMismatch("payload-count")
        }
        ...
        for descriptor in descriptors {
            ...
            guard declared.insert(descriptor.relativePath).inserted else {
                throw SecretGramArchiveValidationError.duplicatePath(descriptor.relativePath)
            }
            guard let data = payloads[descriptor.relativePath] else {
                throw SecretGramArchiveValidationError.missingPayload(descriptor.relativePath)
            }
            total += Int64(data.count)
            guard Int64(data.count) == descriptor.uncompressedBytes else {
                throw SecretGramArchiveValidationError.sizeMismatch(descriptor.relativePath)
            }
            guard SecretGramSHA256.hex(data) == descriptor.sha256.lowercased() else {
                throw SecretGramArchiveValidationError.checksumMismatch(descriptor.relativePath)
            }
        }
        guard total <= self.maximumUncompressedBytes else {
            throw SecretGramArchiveValidationError.sizeMismatch("total")
        }
        if let undeclared = payloads.keys.first(where: { !declared.contains($0) }) {
            throw SecretGramArchiveValidationError.undeclaredPayload(undeclared)
        }
    }
}

public enum SecretGramArchiveV1Migrator {
...
// Foundation-only SHA-256 keeps SecretGramCore usable from TelegramCore without
// adding a platform crypto dependency.
public enum SecretGramSHA256 {
