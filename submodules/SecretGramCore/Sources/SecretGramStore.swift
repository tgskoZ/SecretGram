...
public protocol SecretGramEventStore {
    func append(_ event: SecretGramCanonicalEvent) throws
    func events(accountPeerId: Int64, chatPeerId: Int64?) throws -> [SecretGramCanonicalEvent]
    func replaceAtomically(accountPeerId: Int64, events: [SecretGramCanonicalEvent]) throws
}

public final class SecretGramJSONLEventStore: SecretGramEventStore {
    private struct AccountIndexState {
        var records: [SecretGramTimeMachineIndexRecord]
        var recordsByEventId: [SecretGramEventId: SecretGramTimeMachineIndexRecord]
        var recordsByChat: [Int64: [SecretGramTimeMachineIndexRecord]]
    ...
    public func append(_ event: SecretGramCanonicalEvent) throws {
        try self.appendBatch([event])
    }

    public func appendBatch(_ events: [SecretGramCanonicalEvent]) throws {
        try self.appendBatch(events, allowExistingIdentical: false)
    }

    public func appendBatchRecovering(_ events: [SecretGramCanonicalEvent]) throws {
        try self.appendBatch(events, allowExistingIdentical: true)
    }

    private func appendBatch(
        _ events: [SecretGramCanonicalEvent],
        ...
        var grouped: [Int64: [SecretGramCanonicalEvent]] = [:]
        for event in events {
    ...
    ) throws -> [SecretGramTimeMachineIndexRecord] {
        Self.sharedLock.lock()
        defer { Self.sharedLock.unlock() }
    ...
    public func readyIndexRecords(
        ...
        guard let state = try self.readyIndexLocked(accountPeerId) else {
            throw SecretGramCoreError.indexNotReady(accountPeerId)
        }
        return self.filteredRecords(
            state.records,
            chatPeerId: chatPeerId,
    ...
    private func filteredRecords(
        _ records: [SecretGramTimeMachineIndexRecord],
        chatPeerId: Int64,
        afterSequence: Int64?,
        throughSequence: Int64?
    ) -> [SecretGramTimeMachineIndexRecord] {
    ...
    public func events(accountPeerId: Int64, chatPeerId: Int64?) throws -> [SecretGramCanonicalEvent] {
        Self.sharedLock.lock()
        defer { Self.sharedLock.unlock() }
        if let chatPeerId {
            let state = try self.ensureIndexLocked(accountPeerId)
    ...
    public func eventPage(
        ...
        beforeEventId: SecretGramEventId? = nil,
        limit: Int = 250
    ) throws -> [SecretGramCanonicalEvent] {
        Self.sharedLock.lock()
        defer { Self.sharedLock.unlock() }
    ...
    public func replaceAtomically(accountPeerId: Int64, events: [SecretGramCanonicalEvent]) throws {
        Self.sharedLock.lock()
        defer { Self.sharedLock.unlock() }
        precondition(events.allSatisfy { $0.accountPeerId == accountPeerId })
        var identities = Set<SecretGramEventId>()
        for event in events {
            guard identities.insert(event.eventId).inserted else {
                throw SecretGramCoreError.duplicateEvent(event.eventId)
            }
        }
    ...
    private func appendLocked(
        ...
        var pendingIds = Set<SecretGramEventId>()
        var newEvents: [SecretGramCanonicalEvent] = []
        for event in events {
            ...
            if let existingRecord = state.recordsByEventId[event.eventId] {
                ...
                if existing == event {
                    ...
                    throw SecretGramCoreError.conflictingEvent(event.eventId)
                }
            }
        ...
        var newRecords: [SecretGramTimeMachineIndexRecord] = []
    ...
    private func buildIndexState(accountPeerId: Int64, canonicalData: Data) throws -> AccountIndexState {
        var records: [SecretGramTimeMachineIndexRecord] = []
        var identities: [SecretGramEventId: SecretGramTimeMachineIndexRecord] = [:]
        var completeLength: UInt64 = 0
        for range in self.completeLineRanges(canonicalData) {
            let line = canonicalData.subdata(in: range)
            let event = try self.decoder.decode(SecretGramCanonicalEvent.self, from: line)
            guard event.schemaVersion == 1 else {
                throw SecretGramCoreError.unsupportedSchemaVersion(event.schemaVersion)
            }
            guard event.accountPeerId == accountPeerId else {
                throw SecretGramCoreError.accountScopeMismatch(
    ...
    private func groupedRecordsByChat(
        _ records: [SecretGramTimeMachineIndexRecord]
    ) -> [Int64: [SecretGramTimeMachineIndexRecord]] {
        var result: [Int64: [SecretGramTimeMachineIndexRecord]] = [:]
    ...
    private func mergeAscending(
        _ lhs: [SecretGramTimeMachineIndexRecord],
        _ rhs: [SecretGramTimeMachineIndexRecord]
    ) -> [SecretGramTimeMachineIndexRecord] {
        var result: [SecretGramTimeMachineIndexRecord] = []
    ...
    private func pageUpperBound(
        ...
        beforeEventId: SecretGramEventId?
    ) -> Int {
    ...
    private func indexRecord(
        event: SecretGramCanonicalEvent,
        byteOffset: UInt64,
        byteLength: UInt64
    ) throws -> SecretGramTimeMachineIndexRecord {
        return SecretGramTimeMachineIndexRecord(
            ...
            locator: try SecretGramCanonicalLocator(
                kind: event.kind,
    ...
    private func readEvent(
        accountPeerId: Int64,
        record: SecretGramTimeMachineIndexRecord
    ) throws -> SecretGramCanonicalEvent {
        let handle = try FileHandle(forReadingFrom: self.accountURL(accountPeerId))
        defer { try? handle.close() }
    ...
    private func readEvent(
        ...
        guard record.byteLength <= UInt64(Int.max) else {
            throw SecretGramCoreError.invalidIndexRange
        }
        ...
        guard data.count == Int(record.byteLength) else {
            throw SecretGramCoreError.incompleteRead(
                expected: Int(record.byteLength),
                actual: data.count
            )
        }
        let event = try self.decoder.decode(SecretGramCanonicalEvent.self, from: data)
        guard event.schemaVersion == 1 else {
            throw SecretGramCoreError.unsupportedSchemaVersion(event.schemaVersion)
        }
        return event
    ...
    private func loadAccount(_ accountPeerId: Int64) throws -> [SecretGramCanonicalEvent] {
        let url = self.accountURL(accountPeerId)
        guard FileManager.default.fileExists(atPath: url.path) else { return [] }
        let data = try Data(contentsOf: url, options: .mappedIfSafe)
        return try self.completeLineRanges(data).map { range in
            let event = try self.decoder.decode(SecretGramCanonicalEvent.self, from: data.subdata(in: range))
            guard event.schemaVersion == 1 else {
                throw SecretGramCoreError.unsupportedSchemaVersion(event.schemaVersion)
            }
            return event
    ...
    private func loadIndex(accountPeerId: Int64) throws -> [SecretGramTimeMachineIndexRecord] {
        let url = self.indexURL(accountPeerId)
        let data = try Data(contentsOf: url, options: .mappedIfSafe)
        return try self.completeLineRanges(data).map { range in
            try self.decoder.decode(SecretGramTimeMachineIndexRecord.self, from: data.subdata(in: range))
        }
    }

    private func appendIndexLocked(accountPeerId: Int64, records: [SecretGramTimeMachineIndexRecord]) throws {
        let url = self.indexURL(accountPeerId)
    ...
    private func publishIndexLocked(accountPeerId: Int64, records: [SecretGramTimeMachineIndexRecord]) throws {
        let url = self.indexURL(accountPeerId)
    ...
    private func canonicalData(events: [SecretGramCanonicalEvent]) throws -> Data {
        var data = Data()
        for event in events {
...
public enum SecretGramCaptureRecorder {
    private static let queueKey = DispatchSpecificKey<Void>()
    private static let queue: DispatchQueue = {
        let queue = DispatchQueue(label: "secretgram.capture.recorder", qos: .utility)
        queue.setSpecific(key: SecretGramCaptureRecorder.queueKey, value: ())
        return queue
    }()
    ...
    private static let store = SecretGramJSONLEventStore(rootURL: rootURL)
    private static var sequenceCounter: Int64 = 0
    private static var pendingEvents: [SecretGramCanonicalEvent] = []
    ...
    private static let lifecycleObservers: [NSObjectProtocol] = [
        NotificationCenter.default.addObserver(
            ...
            object: nil,
            queue: nil,
            using: { _ in SecretGramCaptureRecorder.requestLifecycleFlush() }
        ),
        NotificationCenter.default.addObserver(
    ...
    public static func record(
        ...
        let eventId = SecretGramEventId.random()
        _ = self.lifecycleObservers
        self.queue.async {
            self.sequenceCounter += 1
            let sequence = max(observedAtMs * 1_000, observedAtMs * 1_000 + self.sequenceCounter)
            self.pendingEvents.append(SecretGramCanonicalEvent(
                accountPeerId: accountPeerId,
                chatPeerId: chatPeerId,
                eventId: eventId,
                sequence: sequence,
            ...
            if self.pendingEvents.count > self.maximumPendingEvents {
                ...
                NSLog("[SecretGram capture] dropped %d oldest events after persistent storage failure", overflow)
            }
    ...
    private static func flush(scheduleContinuation: Bool = true) -> Bool {
        ...
        var failedEvents: [SecretGramCanonicalEvent] = []
        for accountEvents in eventsByAccount.values {
            do {
                ...
                NSLog("[SecretGram capture] account append failed: %@", String(describing: error))
            }
        }
