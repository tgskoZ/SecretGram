...
static NSString *SecretGramDiagnosticsDirectory(NSString *sharedContainerPath) {
    return [sharedContainerPath stringByAppendingPathComponent:@"telegram-data/secretgram-extension-diagnostics"];
}

static NSString *SecretGramResolvedDiagnosticsGroup(void) {
...
@implementation BuildConfig (SecretGramExtensionDiagnostics)

+ (void)secretgramRecordExtensionDiagnosticWithProcess:(NSString *)process
    ...
    NSLog(@"[SecretGramExtension] %@ %@ group=%@ path=%@ %@", process, stage, appGroupIdentifier, sharedContainerPath, boundedDetail);
    if (sharedContainerPath.length == 0) {
        return;
    }
    NSString *directory = SecretGramDiagnosticsDirectory(sharedContainerPath);
...
+ (NSString *)secretgramExtensionDiagnosticsReport {
    NSString *group = SecretGramResolvedDiagnosticsGroup();
    ...
    NSString *directory = SecretGramDiagnosticsDirectory(container.path);
...
+ (NSString *)secretgramExtensionContainerClassificationForPath:(NSString *)path {
    if (path.length == 0) {
        return @"missing";
    }
...
+ (NSString *)secretgramExtensionBoundarySummaryWithProcess:(NSString *)process
    stage:(NSString *)stage
    path:(NSString *)path {
    NSString *classification = [self secretgramExtensionContainerClassificationForPath:path];
    NSString *safeProcess = process.length == 0 ? @"extension" : process;
    NSString *safeStage = stage.length == 0 ? @"unknown" : stage;
    return [NSString stringWithFormat:
        @"SecretGram %@: %@ failed (container=%@). Open the main app once, then retry. If this remains processLocal, the signer isolated the App Group.",
