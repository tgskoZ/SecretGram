...
private struct SecretGramActivityGhostSettings {
    let typing: Bool
    let recording: Bool
    let uploading: Bool
    let sticker: Bool
...
public enum SecretGramActivityGhostRuntime {
    private static let cached = Atomic(value: SecretGramActivityGhostSettings?.none)

    public static func invalidate() {
        let _ = self.cached.modify { _ in nil }
    }

    private static func settings() -> SecretGramActivityGhostSettings {
        ...
        return self.cached.modify { cached in
            ...
            let loaded = SecretGramActivityGhostSettings(
                typing: value("secretgram.GhostMode.TypingActions"),
                recording: value("secretgram.GhostMode.HideRecording"),
                uploading: value("secretgram.GhostMode.HideUploading"),
                sticker: value("secretgram.GhostMode.HideStickerActivity"),
                game: value("secretgram.GhostMode.HideGameActivity"),
                emoji: value("secretgram.GhostMode.HideEmojiActivity")
            )
            return loaded
        }!
    }
...
public enum SecretGramHotSettings {
...
private func ghostBaseBotSafeModeEnabled(_ peerId: PeerId) -> Bool {
    return UserDefaults.standard.bool(
        forKey: "secretgram.BotAccount.\(peerId.toInt64())"
    )
}
...
private func ghostBaseBotSafeRecord(
    ...
    UserDefaults.standard.set(value, forKey: "secretgram.BotSafe.LastEvent")
    UserDefaults.standard.set(
        value,
        forKey: "secretgram.BotSafe.LastEvent.\(peerId.toInt64())"
    )
...
public class Account {
    ...
    public func updateLocalInputActivity(peerId: PeerActivitySpace, activity: PeerInputActivity, isPresent: Bool) {
        if isPresent && SecretGramActivityGhostRuntime.shouldSuppress(activity) {
            return
        }
