...
private enum SecretGramBuild133UnseenTargetKind {
    case mention
    case reaction
}

private func secretgramBuild133IsNavigableUnseenTarget(
    accountPeerId: PeerId,
    message: Message,
    kind: SecretGramBuild133UnseenTargetKind
) -> Bool {
...
private func secretgramBuild133FirstNavigableUnseenTarget(
    accountPeerId: PeerId,
    entries: [MessageHistoryEntry],
    kind: SecretGramBuild133UnseenTargetKind
) -> Message? {
    for entry in entries {
        if secretgramBuild133IsNavigableUnseenTarget(
            accountPeerId: accountPeerId,
...
func _internal_earliestUnseenPersonalMentionMessage(account: Account, peerId: PeerId, threadId: Int64?) -> Signal<EarliestUnseenPersonalMentionMessageResult, NoError> {
    ...
    |> mapToSignal { view -> Signal<EarliestUnseenPersonalMentionMessageResult, NoError> in
        ...
        if let message = secretgramBuild133FirstNavigableUnseenTarget(
