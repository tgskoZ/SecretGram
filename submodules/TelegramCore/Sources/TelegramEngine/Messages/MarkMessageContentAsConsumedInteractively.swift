                ...
                if let attribute = updatedAttributes[i] as? ConsumableContentMessageAttribute {
                    if !attribute.consumed {
                        ...
                        if message.id.peerId.namespace == Namespaces.Peer.SecretChat {
                            ...
                            let ghostBaseReadGhostExtras = (UserDefaults.standard.object(forKey: "secretgram.GhostMode.ReadMessages") as? Bool) ?? false
...
func markMessageContentAsConsumedRemotely(transaction: Transaction, messageId: MessageId, consumeDate: Int32?) {
    if let message = transaction.getMessage(messageId) {
        ...
        let secretgramKeepOneTimeRemoteMedia = (
        ...
        for i in 0 ..< updatedAttributes.count {
            ...
            if let attribute = updatedAttributes[i] as? AutoremoveTimeoutMessageAttribute {
                if (attribute.countdownBeginTime == nil || attribute.countdownBeginTime == 0) && message.containsSecretMedia {
                    if secretgramKeepOneTimeRemoteMedia {
                    ...
                    if message.id.peerId.namespace == Namespaces.Peer.SecretChat {
                        ...
                        if !secretgramKeepOneTimeRemoteMedia && (attribute.timeout == viewOnceTimeout || timestamp >= countdownBeginTime + attribute.timeout) {
                            for i in 0 ..< updatedMedia.count {
                                if let _ = updatedMedia[i] as? TelegramMediaImage {
                                    let ghostBaseOT1KeepOutgoingTimerLocal = (((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.Enabled") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.OneTimeSave") as? Bool) ?? false) && message.id.peerId.namespace != Namespaces.Peer.SecretChat)
                                    ...
                                    let ghostBaseKeepVoiceCircleLocal = (((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.Enabled") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.OneTimeSave") as? Bool) ?? false) && message.id.peerId.namespace != Namespaces.Peer.SecretChat && (file.isInstantVideo || file.isVoice))
                                    let ghostBaseOT1KeepOutgoingTimerLocal = (((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.Enabled") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.OneTimeSave") as? Bool) ?? false) && message.id.peerId.namespace != Namespaces.Peer.SecretChat)
