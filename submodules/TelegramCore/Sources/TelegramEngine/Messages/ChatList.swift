...
public final class EngineChatList: Equatable {
    ...
    public final class Item: Equatable {
        ...
        public let secretgramPresentationIndex: Index?
        ...
        public init(
            ...
            secretgramPresentationIndex: Index? = nil
        ) {
            self.id = id
            self.index = index
            self.secretgramPresentationIndex = secretgramPresentationIndex
        ...
        public static func ==(lhs: Item, rhs: Item) -> Bool {
            ...
            if lhs.secretgramPresentationIndex != rhs.secretgramPresentationIndex {
                return false
            }
...
public extension EngineChatList.Item {
    func withUpdatedCommunitySummary(messages: [EngineMessage], readCounters: EnginePeerReadCounters?, secretgramPresentationIndex: EngineChatList.Item.Index? = nil) -> EngineChatList.Item {
        ...
        return EngineChatList.Item(
            ...
            secretgramPresentationIndex: secretgramPresentationIndex
        )
    }
}
...
private func secretgramBuild133ActivityVisible(
    stock: Bool,
    expectedCount: Int,
...
private func secretgramBuild133ReactionActivityHidden(
    accountPeerId: PeerId,
    message: Message
) -> Bool {
    ...
    return !SecretGramBlockedReactionPolicy.hasVisibleUnseenReaction(
        accountPeerId: accountPeerId,
        message: message,
        attribute: attribute
    )
        ...
        case let .MessageEntry(entryData):
            ...
            if let accountPeerId, let chatPeer = renderedPeer.chatMainPeer {
                visibleMessages = messages.filter { message in
                    return !SecretGramBlockedReactionPolicy.isMessageHidden(
                        accountPeerId: accountPeerId,
                        chatPeer: chatPeer,
                        message: message
                    )
            ...
            if let info = tagSummaryInfo[ChatListEntryMessageTagSummaryKey(
                ...
                hasUnseenMentions = secretgramBuild133ActivityVisible(
            ...
            if let info = tagSummaryInfo[ChatListEntryMessageTagSummaryKey(
                ...
                hasUnseenReactions = secretgramBuild133ActivityVisible(
                    stock: stockHasUnseenReactions,
                    expectedCount: outstandingCount,
                    accountPeerId: accountPeerId,
                    messages: messages,
                    tag: .unseenReaction,
                    hidden: secretgramBuild133ReactionActivityHidden
                )
            }
