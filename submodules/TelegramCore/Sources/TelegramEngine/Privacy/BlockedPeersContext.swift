...
public enum SecretGramBlockedReactionPolicy {
    public static let hideBlockedReactionsKey = "secretgram.Messages.HideBlockedReactions"
    public static let hideBlockedMessagesKey = "secretgram.Messages.HideBlockedMessages"

    private static let blockedPeerIdsByAccount = Atomic(value: [PeerId: Set<PeerId>]())

    private struct SecretGramBuild137BlockedSettings {
        let hideReactions: Bool
        let hideMessages: Bool
    }
    private static let settingsByAccount = Atomic(value: [PeerId: SecretGramBuild137BlockedSettings]())

    private static func settings(accountPeerId: PeerId) -> SecretGramBuild137BlockedSettings {
        let result = self.settingsByAccount.modify { current in
            ...
            func value(_ key: String) -> Bool {
                let scopedKey = "secretgram.account.\(accountPeerId.toInt64()).setting.\(key)"
            ...
            let loaded = SecretGramBuild137BlockedSettings(
...
public final class SecretGramBlockedPeersObserver {
    ...
    public init(account: Account) {
        ...
        |> deliverOnMainQueue).start(next: { state in
            SecretGramBlockedReactionPolicy.replaceBlockedPeerIds(
