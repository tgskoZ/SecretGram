...
public extension EngineMessageReactionListContext.State {
    init(message: EngineMessage, readStats: MessageReadStats?, reaction: MessageReaction.Reaction?, accountPeerId: PeerId? = nil) {
        ...
        let reactionsAttribute: ReactionsMessageAttribute?
        if let accountPeerId {
            reactionsAttribute = secretgramFilteredMessageReactions(
...
public final class EngineMessageReactionListContext {
    ...
    private final class Impl {
        ...
        init(queue: Queue, account: Account, message: EngineMessage, readStats: MessageReadStats?, reaction: MessageReaction.Reaction?) {
            ...
            let initialState = EngineMessageReactionListContext.State(message: message, readStats: readStats, reaction: reaction, accountPeerId: account.peerId)
                ...
                |> mapToSignal { result -> Signal<InternalState, NoError> in
                    return account.postbox.transaction { transaction -> InternalState in
                        ...
                        case let .messageReactionsList(messageReactionsListData):
                            ...
                            let filterBlocked = SecretGramBlockedReactionPolicy.isGroupMessage(message._asMessage())
                                && SecretGramBlockedReactionPolicy.hideBlockedReactions(accountPeerId: accountPeerId)
                            for reaction in reactions {
                                switch reaction {
                                case let .messagePeerReaction(messagePeerReactionData):
                                    ...
                                    if filterBlocked && peerId != accountPeerId && SecretGramBlockedReactionPolicy.isBlocked(accountPeerId: accountPeerId, peerId: peerId) {
