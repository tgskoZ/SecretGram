...
public final class AccountStateManager {
    ...
    private final class Impl {
        ...
        private func ghostBaseBotShadowPrefix() -> String {
            return "secretgram.BotShadow1.\(self.accountPeerId.toInt64())"
        }
        ...
        private func ghostBaseBotShadowPage(cursor: GhostBaseBotShadowCursor, importedTotal: Int) -> Signal<Bool, NoError> {
            ...
            |> `catch` { error -> Signal<Api.updates.Difference?, NoError> in
                ...
                Logger.shared.log("secretgram.BotShadow1", "getDifference failed \(error.errorCode) \(error.errorDescription ?? "nil")")
                self.ghostBaseBotShadowRelease()
                return .single(nil)
            }
            |> mapToSignal { difference -> Signal<Bool, NoError> in
                ...
                case let .difference(data):
                    ...
                    |> map { added in
                        ...
                        self.ghostBaseBotShadowRelease()
                        Logger.shared.log("secretgram.BotShadow1", "final added=\(total)")
                        return total > 0
                    }
                ...
                case .differenceTooLong:
                    ...
                    self.ghostBaseBotShadowRelease()
                    Logger.shared.log("secretgram.BotShadow1", "differenceTooLong; no state mutation and no dialog RPC")
                    return .single(false)
                }
            }
        }
