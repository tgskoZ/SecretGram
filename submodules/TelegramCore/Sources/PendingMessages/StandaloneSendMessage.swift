...
private func ghostBaseV10SApplyStandaloneSchedule(
    ...
    let standardEnabled = (
        standardDefaults.object(
            forKey: "secretgram.GhostMode.ScheduledSend"
        ) as? Bool
    ) ?? false
    ...
    let counterKey =
        "secretgram.SH2.StandaloneScheduledIntercept.Count"
    ...
    standardDefaults.set(
        "\(peerId)",
        forKey: "secretgram.SH2.LastStandalonePeerId"
    )

    standardDefaults.set(
        Int(scheduleTime),
        forKey: "secretgram.SH2.LastStandaloneScheduleTime"
    )
}
