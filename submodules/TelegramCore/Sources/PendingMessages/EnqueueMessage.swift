...
private enum GhostBasePublicPeerNameStore {
    private static let valuesKey = "secretgram.PublicPeerNames.V11T"
    private static let orderKey = "secretgram.PublicPeerNamesOrder.V11T"
...
private func ghostBaseDeletedMediaLabel(_ message: Message) -> String? {
    ...
    // richer UI localization uses semantic SecretGramStringKey values.
        ...
        return Signal { subscriber in
            DispatchQueue.global(qos: .utility).async {
                ...
                for plan in plans {
                    ...
                    let secretgramStickerReply = source.media.contains { media in
                        guard let file = media as? TelegramMediaFile else {
                            return false
                        }
                        return file.isSticker
                    }
                    let recovered = secretgramStickerReply ? nil : recoveredGroup.first
