...
public func secretgramFilteredMessageReactions(
    ...
    guard SecretGramBlockedReactionPolicy.isGroupMessage(message) else {
        return attribute
    }
    guard SecretGramBlockedReactionPolicy.hideBlockedReactions(accountPeerId: accountPeerId) else {
        return attribute
    }
    let blockedPeerIds = SecretGramBlockedReactionPolicy.blockedPeerIds(accountPeerId: accountPeerId)
...
public func secretgramVisibleMessageReactions(
    accountPeerId: EnginePeer.Id,
    message: Message
) -> ReactionsMessageAttribute? {
    return secretgramFilteredMessageReactions(
...
public func mergedMessageReactionsAndPeers(accountPeerId: EnginePeer.Id, accountPeer: EnginePeer?, message: Message) -> (reactions: [MessageReaction], peers: [(MessageReaction.Reaction, EnginePeer)]) {
    guard let attribute = secretgramVisibleMessageReactions(accountPeerId: accountPeerId, message: message) else {
        return ([], [])
    }
