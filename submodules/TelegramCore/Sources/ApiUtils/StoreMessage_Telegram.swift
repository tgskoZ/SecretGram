...
private func secretgramBuild133FilteredActivityTags(
    ...
    if tags.contains(.unseenPersonalMessage),
       SecretGramBlockedReactionPolicy.hideBlockedMessages(accountPeerId: accountPeerId),
       let authorId,
       authorId != accountPeerId,
       SecretGramBlockedReactionPolicy.isBlocked(accountPeerId: accountPeerId, peerId: authorId) {
    ...
    if tags.contains(.unseenReaction),
       ...
       !SecretGramBlockedReactionPolicy.hasVisibleUnseenReactionInGroup(
    ...
    convenience init?(apiMessage: Api.Message, accountPeerId: PeerId, peerIsForum: Bool, namespace: MessageId.Namespace = Namespaces.Message.Cloud) {
        switch apiMessage {
            case let .message(messageData):
                ...
                let (secretgramBuild133RawTags, globalTags) = tagsForStoreMessage(incoming: storeFlags.contains(.Incoming), attributes: attributes, media: medias, textEntities: entitiesAttribute?.entities, isPinned: isPinned)
                let tags = secretgramBuild133FilteredActivityTags(
                    accountPeerId: accountPeerId,
                    chatPeerId: peerId,
                    authorId: authorId,
                    attributes: attributes,
                    tags: secretgramBuild133RawTags
                )
            ...
            case let .messageService(messageServiceData):
                ...
                let (secretgramBuild133RawTags, globalTags) = tagsForStoreMessage(incoming: storeFlags.contains(.Incoming), attributes: attributes, media: media, textEntities: nil, isPinned: false)
                let tags = secretgramBuild133FilteredActivityTags(
                    accountPeerId: accountPeerId,
                    chatPeerId: peerId,
                    authorId: authorId,
