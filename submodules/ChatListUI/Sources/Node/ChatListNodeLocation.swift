...
private func secretgramBuild134ChatListPresentationUpdates<T>(
    _ signal: Signal<T, NoError>
) -> Signal<T, NoError> {
    return combineLatest(
        signal,
        SecretGramBlockedReactionPolicy.presentationUpdates
    )
    |> map { value, _ in
        return value
    }
...
private struct SecretGramBuild136VisibilityCacheKey: Hashable {
    let accountPeerId: PeerId
    let peerId: PeerId
}

private struct SecretGramBuild136VisibilityCacheEntry {
...
private struct SecretGramBuild136VisibilityCacheState {
    var entries: [SecretGramBuild136VisibilityCacheKey: SecretGramBuild136VisibilityCacheEntry] = [:]
    var insertionOrder: [SecretGramBuild136VisibilityCacheKey] = []
}

private let secretgramBuild136VisibilityCache = Atomic(
    value: SecretGramBuild136VisibilityCacheState()
)

private let secretgramBuild136VisibilityCacheLimit = 256

private func secretgramBuild136ClearVisibilityCache(accountPeerId: PeerId) {
    let _ = secretgramBuild136VisibilityCache.modify { current in
        var current = current
...
private func secretgramBuild136StoreVisibilityCache(
    _ additions: [SecretGramBuild136VisibilityCacheKey: SecretGramBuild136VisibilityCacheEntry],
    removing removals: Set<SecretGramBuild136VisibilityCacheKey>
) {
    ...
    let _ = secretgramBuild136VisibilityCache.modify { current in
        ...
        while current.insertionOrder.count > secretgramBuild136VisibilityCacheLimit {
...
private func secretgramBuild136ReadCounters(
...
private func secretgramBuild136PresentationIndex(
    sourceIndex: EngineChatList.Item.Index,
    cached: SecretGramBuild136VisibilityCacheEntry?
...
private func secretgramBuild136Applying(
    _ cached: SecretGramBuild136VisibilityCacheEntry,
    ...
    return item.withUpdatedCommunitySummary(
        messages: messages,
        readCounters: secretgramBuild136ReadCounters(
            item.readCounters,
            subtracting: cached.hiddenUnreadCount
        ),
        secretgramPresentationIndex: secretgramBuild136PresentationIndex(
...
private func secretgramBuild136VisibleChatListUpdate(
    account: Account,
    update: ChatListNodeViewUpdate
) -> Signal<ChatListNodeViewUpdate, NoError> {
    guard SecretGramBlockedReactionPolicy.hideBlockedMessages(accountPeerId: account.peerId) else {
        secretgramBuild136ClearVisibilityCache(accountPeerId: account.peerId)
        return .single(update)
    }
    let blockedPeerIds = SecretGramBlockedReactionPolicy.blockedPeerIds(accountPeerId: account.peerId)
    guard !blockedPeerIds.isEmpty else {
        secretgramBuild136ClearVisibilityCache(accountPeerId: account.peerId)
        return .single(update)
    }

    let presentationRevision = SecretGramBlockedReactionPolicy.presentationRevisionValue
    let cachedEntries = secretgramBuild136VisibilityCache.with { current in current.entries }
    var reusable: [PeerId: SecretGramBuild136VisibilityCacheEntry] = [:]
    ...
    for item in update.list.items {
        guard case let .chatList(peerId) = item.id,
              ...
              SecretGramBlockedReactionPolicy.isGroupChat(peer._asPeer()),
              item.messages.isEmpty || (item.readCounters?.count ?? 0) > 0 else {
            continue
        }
        let key = SecretGramBuild136VisibilityCacheKey(accountPeerId: account.peerId, peerId: peerId)
    ...
    let buildUpdate: ([PeerId: SecretGramBuild136VisibilityCacheEntry]) -> ChatListNodeViewUpdate = { resolved in
        let items = update.list.items.map { item -> EngineChatList.Item in
            guard case let .chatList(peerId) = item.id,
                  let cached = resolved[peerId] else {
                return item
            }
            return secretgramBuild136Applying(cached, to: item)
        }.sorted { lhs, rhs in
            let lhsIndex = lhs.secretgramPresentationIndex ?? lhs.index
            let rhsIndex = rhs.secretgramPresentationIndex ?? rhs.index
    ...
    return account.postbox.transaction { transaction -> ChatListNodeViewUpdate in
        ...
        var newCacheEntries: [SecretGramBuild136VisibilityCacheKey: SecretGramBuild136VisibilityCacheEntry] = [:]
        var removedCacheKeys = Set<SecretGramBuild136VisibilityCacheKey>()
        for item in update.list.items {
            guard case let .chatList(peerId) = item.id,
                  missingPeerIds.contains(peerId),
                  let chatPeer = transaction.getPeer(peerId),
                  SecretGramBlockedReactionPolicy.isGroupChat(chatPeer) else {
                continue
            }
            ...
            let cacheKey = SecretGramBuild136VisibilityCacheKey(
            ...
            for entry in historyView.entries where !entry.isRead && entry.message.flags.contains(.Incoming) {
                if SecretGramBlockedReactionPolicy.isMessageHidden(
                    accountPeerId: account.peerId,
                    chatPeer: chatPeer,
            ...
            let fallbackMessage = historyView.entries.reversed().first(where: { entry in
                !SecretGramBlockedReactionPolicy.isMessageHidden(
                    accountPeerId: account.peerId,
                    chatPeer: chatPeer,
                    message: entry.message
                )
            }).map { EngineMessage($0.message) }
            let cached = SecretGramBuild136VisibilityCacheEntry(
                sourceIndex: item.index,
        ...
        secretgramBuild136StoreVisibilityCache(newCacheEntries, removing: removedCacheKeys)
...
public func chatListViewForLocation(chatListLocation: ChatListControllerLocation, location: ChatListNodeLocation, account: Account, shouldLoadCanMessagePeer: Bool) -> Signal<ChatListNodeViewUpdate, NoError> {
    ...
    case let .chatList(groupId):
        ...
        case let .initial(count, _):
            ...
            return secretgramBuild134ChatListPresentationUpdates(signal)
            ...
            |> mapToSignal { update -> Signal<ChatListNodeViewUpdate, NoError> in
                ...
                |> mapToSignal { summarized in
                    return secretgramBuild136VisibleChatListUpdate(account: account, update: summarized)
                }
            }
        case let .navigation(index, _):
            ...
            var first = true
            return secretgramBuild134ChatListPresentationUpdates(
