...
final class ChatImageGalleryItemNode: ZoomableContentGalleryItemNode {
    ...
    fileprivate func setMessage(_ message: Message, mediaSubject: GalleryMediaSubject?, displayInfo: Bool, translateToLanguage: String?, peerIsCopyProtected: Bool, isSecret: Bool, location: MessageHistoryEntryLocation?) {
        ...
        self.peerIsCopyProtected = peerIsCopyProtected
        self.isSecret = isSecret
        self.imageNode.captureProtected = message.id.peerId.namespace == Namespaces.Peer.SecretChat || isSecret || message.paidContent != nil || (!(((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.AllowScreenshots") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.AllowScreenRecording") as? Bool) ?? true)) && (message.isCopyProtected() || peerIsCopyProtected))
    ...
    private func contextMenuMainItems() -> Signal<[ContextMenuItem], NoError> {
        ...
        |> map { [weak self] peer -> [ContextMenuItem] in
            ...
            if let message = self.message {
                ...
                let ghostBaseProtectedEnabled = ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.Enabled") as? Bool) ?? true)
                let ghostBaseProtectedSave = ghostBaseProtectedEnabled && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.GallerySave") as? Bool) ?? true)
                let ghostBaseProtectedCopy = ghostBaseProtectedEnabled && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.GalleryCopy") as? Bool) ?? true)
                let ghostBaseProtectedOriginalAllowed = !message.isCopyProtected() && !self.peerIsCopyProtected

                let ghostBaseTimedImageSave = ghostBaseProtectedEnabled && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.OneTimeSave") as? Bool) ?? false) && message.id.peerId.namespace != Namespaces.Peer.SecretChat && message.paidContent == nil && message.minAutoremoveOrClearTimeout != nil
