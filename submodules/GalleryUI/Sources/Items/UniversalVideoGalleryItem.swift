    ...
    private func contextMenuMainItems(isSettings: Bool, dismiss: @escaping () -> Void) -> Signal<(items: [ContextMenuItem], topItems: [ContextMenuItem]), NoError> {
        ...
        |> map { [weak self] status, peer, videoQualityState -> (items: [ContextMenuItem], topItems: [ContextMenuItem]) in
            ...
            if isSettings {
                ...
                let ghostBaseProtectedVideoSave = (((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.Enabled") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.GallerySave") as? Bool) ?? true))
                if let (message, maybeFile, _) = strongSelf.contentInfo(), let file = maybeFile {
                    let ghostBaseTimedVideoSave = (((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.Enabled") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.OneTimeSave") as? Bool) ?? false) && message.id.peerId.namespace != Namespaces.Peer.SecretChat && message.paidContent == nil && message.minAutoremoveOrClearTimeout != nil)
