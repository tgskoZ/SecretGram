    ...
    present: @escaping (ViewController, Any?) -> Void) -> GalleryItem?
{
    ...
    if let image = media as? TelegramMediaImage {
        if let file = image.video {
            let captureProtected = (message.containsSecretMedia && message.minAutoremoveOrClearTimeout != viewOnceTimeout) || (message.minAutoremoveOrClearTimeout == viewOnceTimeout && !(((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.OneTimeScreenshots") as? Bool) ?? false) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.OneTimeScreenRecording") as? Bool) ?? false) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.Enabled") as? Bool) ?? true) && message.id.peerId.namespace != Namespaces.Peer.SecretChat)) || message.paidContent != nil || (!(((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.AllowScreenshots") as? Bool) ?? true) && ((SecretGramHotSettings.object(forKey: "secretgram.ProtectedContent.AllowScreenRecording") as? Bool) ?? true)) && (message.isCopyProtected() || peerIsCopyProtected))
