...
private struct SecretGramDataUIState: Equatable {
    var configuration: SecretGramRetentionConfiguration
}

private final class SecretGramDataUIArguments {
...
private enum SecretGramDataUIEntry: ItemListNodeEntry {
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        let arguments = arguments as! SecretGramDataUIArguments
        switch self {
...
private func secretgramDurationTitle(_ value: SecretGramHistoryDuration, strings: SecretGramStrings) -> String {
    switch value {
...
private func secretgramMediaLimitTitle(_ value: SecretGramMediaByteLimit, strings: SecretGramStrings) -> String {
    switch value {
    case .disabled: return strings.disabled
...
private func secretgramDataEntries(state: SecretGramDataUIState, strings: SecretGramStrings) -> [SecretGramDataUIEntry] {
    let policy = state.configuration.accountPolicy
    let summary = strings.build124DataSummary(
        secretgramDurationTitle(policy.historyDuration, strings: strings),
        secretgramMediaLimitTitle(policy.mediaByteLimit, strings: strings),
        state.configuration.accountPeerId
    )
    var entries: [SecretGramDataUIEntry] = [
        .summary(0, 1, strings.dataAndBackup, summary),
        .header(1, strings.retentionRules),
        .action(1, 1, strings.historyDuration, secretgramDurationTitle(policy.historyDuration, strings: strings), "duration"),
        .action(1, 2, strings.recoveredMediaLimit, secretgramMediaLimitTitle(policy.mediaByteLimit, strings: strings), "media"),
...
public func secretgramDataAndBackupController(context: AccountContext) -> ViewController {
    let accountPeerId = context.account.peerId.toInt64()
    let initial = SecretGramDataUIState(
        configuration: SecretGramRetentionRuntime.configuration(accountPeerId: accountPeerId)
    )
    ...
    func update(_ transform: (inout SecretGramRetentionConfiguration) -> Void) {
        let value = stateValue.modify { current in
            var current = current
            transform(&current.configuration)
            try? SecretGramRetentionRuntime.save(current.configuration)
            return current
        }
        statePromise.set(value)
    }

    let durations: [SecretGramHistoryDuration] = [.disabled, .days7, .days30, .days90, .forever]
    let mediaLimits: [SecretGramMediaByteLimit] = [.disabled, .megabytes250, .megabytes500, .gigabytes1, .gigabytes2, .gigabytes5, .unlimited]
    let arguments = SecretGramDataUIArguments(action: { action in
        switch action {
        case "duration":
        ...
        case "export":
            if let controller { secretgramPresentArchiveExport(context: context, controller: controller) }
        case "import":
            if let controller { secretgramPresentArchiveImport(context: context, controller: controller) }
        case "perChat":
            let strings = context.sharedContext.currentPresentationData.with { $0.strings.secretgram }
            ...
            picker.peerSelected = { [weak picker, weak controller] peer, _ in
                picker?.dismiss()
                controller?.push(secretgramChatRetentionController(
                    context: context,
        ...
        case "cleanup":
            Queue.concurrentDefaultQueue().async {
                let store = SecretGramJSONLEventStore(rootURL: secretgramDataCoreRootURL())
                if let events = try? store.events(accountPeerId: accountPeerId, chatPeerId: nil) {
                    let configuration = SecretGramRetentionRuntime.configuration(accountPeerId: accountPeerId)
                    let plan = SecretGramRetentionEngine.cleanupPlan(
                        events: events,
                        policy: configuration.accountPolicy,
                        nowMs: Int64(Date().timeIntervalSince1970 * 1000.0)
                    )
                    try? SecretGramRetentionEngine.applyCleanup(
                        accountPeerId: accountPeerId,
                        originalEvents: events,
                        plan: plan,
                        mediaRootURL: secretgramDataCoreRootURL(),
                        eventStore: store
                    )
                }
            }
    ...
    |> map { presentationData, state -> (ItemListControllerState, (ItemListNodeState, Any)) in
        ...
        return (
            ...
            (ItemListNodeState(
                presentationData: ItemListPresentationData(presentationData),
                entries: secretgramDataEntries(state: state, strings: strings),
...
private func secretgramDataCoreRootURL() -> URL {
    return FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
        .appendingPathComponent("SecretGram", isDirectory: true)
}

private func secretgramChatRetentionController(
    ...
    func update(_ transform: (inout SecretGramChatRetentionOverride) -> Void) {
        let value = stateValue.modify { current in
            var current = current
            var override = current.configuration.chatOverrides.first(where: { $0.chatPeerId == chatPeerId })
                ?? SecretGramChatRetentionOverride(chatPeerId: chatPeerId)
            transform(&override)
    ...
    let durations: [SecretGramHistoryDuration] = [.days7, .days30, .days90, .forever]
    let mediaLimits: [SecretGramMediaByteLimit] = [.disabled, .megabytes250, .megabytes500, .gigabytes1, .gigabytes2, .gigabytes5, .unlimited]
    let arguments = SecretGramDataUIArguments(action: { action in
    ...
    |> map { presentationData, state -> (ItemListControllerState, (ItemListNodeState, Any)) in
        ...
        let entries: [SecretGramDataUIEntry] = [
            .header(0, strings.perChatRules),
            .toggle(0, 1, strings.saveThisChat, "chatCapture", override.captureEnabled ?? true),
            .action(0, 2, strings.historyDuration, secretgramDurationTitle(policy.historyDuration, strings: strings), "chatDuration"),
            .action(0, 3, strings.recoveredMediaLimit, secretgramMediaLimitTitle(policy.mediaByteLimit, strings: strings), "chatMedia"),
