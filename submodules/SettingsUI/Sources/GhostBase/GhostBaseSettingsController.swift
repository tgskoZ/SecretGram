...
private enum GhostBaseKey {
    static let profileEnabled = "secretgram.Profile.Enabled"
    static let showIds = "secretgram.Profile.ShowIds"
    static let showDCs = "secretgram.Profile.ShowDCs"
    static let showRegistration = "secretgram.Profile.ShowRegistration"

    static let readMessages = "secretgram.GhostMode.ReadMessages"
    static let typingActions = "secretgram.GhostMode.TypingActions"
    static let recordingActions = "secretgram.GhostMode.HideRecording"
    static let uploadingActions = "secretgram.GhostMode.HideUploading"
    static let stickerActivity = "secretgram.GhostMode.HideStickerActivity"
    static let gameActivity = "secretgram.GhostMode.HideGameActivity"
    static let emojiActivity = "secretgram.GhostMode.HideEmojiActivity"
    static let presence = "secretgram.GhostMode.Presence"
    static let scheduledSend = "secretgram.GhostMode.ScheduledSend"

    static let saveDeleted = "secretgram.Messages.SaveDeleted"
    static let showDeleted = "secretgram.Messages.ShowDeleted"
    static let saveEditHistory = "secretgram.Messages.SaveEditHistory"
    static let showEditHistory = "secretgram.Messages.ShowEditHistory"
    static let hideBlockedMessages = "secretgram.Messages.HideBlockedMessages"
    static let hideBlockedReactions = "secretgram.Messages.HideBlockedReactions"

    static let glassEnabled = "secretgram.Glass.Enabled"
    static let profileAvatarBlur = "secretgram.ProfileBlur.Avatar"
    static let profileAnimatedBackground = "secretgram.ProfileBlur.Animated"
    static let profileBlurTint = "secretgram.ProfileBlur.Tint"
    static let profileBlurReduced = "secretgram.ProfileBlur.Reduced"
    static let deletedPortableReplies = "secretgram.Messages.DeletedPortableReplies"
    static let preserveDeletedMedia = "secretgram.Messages.PreserveDeletedMedia"
    static let deletedMediaCacheLimit = "secretgram.Messages.DeletedMediaCacheLimit"
    static let deletedMediaRetentionDays = "secretgram.Messages.DeletedMediaRetentionDays"
    static let showRamUnderClock = "secretgram.Appearance.ShowRamUnderClock"
    static let messageSeconds = "secretgram.Appearance.MessageSeconds"
    static let hideOwnPhone = "secretgram.Appearance.HideOwnPhone"

    static let protectedEnabled = "secretgram.ProtectedContent.Enabled"
    static let protectedGalleryShare = "secretgram.ProtectedContent.GalleryShare"
    static let protectedGallerySave = "secretgram.ProtectedContent.GallerySave"
    static let protectedGalleryCopy = "secretgram.ProtectedContent.GalleryCopy"
    static let chatSave = "secretgram.ProtectedContent.ChatSave"
    static let chatCopy = "secretgram.ProtectedContent.ChatCopy"
    static let chatForward = "secretgram.ProtectedContent.ChatForward"
    static let allowScreenshots = "secretgram.ProtectedContent.AllowScreenshots"
    static let allowScreenRecording = "secretgram.ProtectedContent.AllowScreenRecording"
    static let oneTimeScreenshots = "secretgram.ProtectedContent.OneTimeScreenshots"
    static let oneTimeScreenRecording = "secretgram.ProtectedContent.OneTimeScreenRecording"
    static let oneTimeSave = "secretgram.ProtectedContent.OneTimeSave"
    static let storySave = "secretgram.Stories.Save"
    static let localStarsEnabled = "secretgram.Stars.LocalBalance.Enabled"
    static let localStarsAmount = "secretgram.Stars.LocalBalance.Amount"
    static let localStarsBaseAmount = "secretgram.Stars.LocalBalance.BaseAmount"
}
...
private func ghostBaseStarsTransactionPreview(baseAmount: String, targetAmount: String) -> String {
    ...
    let sign = delta >= 0 ? "+" : ""
    return "Transaction Preview: SecretGram \(sign)\(delta)"
}
...
private func secretgramScopedSettingsKey(accountPeerId: Int64, key: String) -> String {
    return "secretgram.account.\(accountPeerId).setting.\(key)"
}

private func secretgramScopedBool(
    accountPeerId: Int64,
    key: String,
    defaultValue: Bool
) -> Bool {
    let scopedKey = secretgramScopedSettingsKey(accountPeerId: accountPeerId, key: key)
...
private func secretgramScopedString(
    accountPeerId: Int64,
    key: String,
    defaultValue: String
) -> String {
...
private enum SecretGramSettingValue: Equatable {
    case bool(Bool)
    case string(String)
...
private enum SecretGramSettingsCommitQueue {
    private static let queue = Queue(name: "SecretGramSettingsCommitQueue", qos: .utility)
    static func enqueue(_ work: @escaping () -> Void) {
        self.queue.async(work)
    }
}

private func secretgramStateValues(_ state: GhostBaseSettingsState) -> [String: SecretGramSettingValue] {
    return [
...
private func secretgramPersistChangedSettings(
    accountPeerId: Int64,
    previous: GhostBaseSettingsState?,
    current: GhostBaseSettingsState
) {
    defer { SecretGramHotSettings.invalidate() }
    let oldValues = previous.map(secretgramStateValues) ?? [:]
    let newValues = secretgramStateValues(current)
    ...
    let secretgramSynchronousRuntimeSettingKeys: Set<String> = Set<String>([
    ...
    ]).union(SecretGramHotSettings.keys)
    let defaults = UserDefaults.standard
    for key in secretgramSynchronousRuntimeSettingKeys {
        guard let value = changes[key] else { continue }
        value.write(
            to: defaults,
            key: secretgramScopedSettingsKey(accountPeerId: accountPeerId, key: key)
        )
        // Legacy Telegram/SecretGram runtime owners consume the active-account
    ...
    let deferredChanges = changes.filter {
        !secretgramSynchronousRuntimeSettingKeys.contains($0.key)
    }
    guard !deferredChanges.isEmpty else { return }

    SecretGramSettingsCommitQueue.enqueue {
        let defaults = UserDefaults.standard
        for (key, value) in deferredChanges {
            value.write(to: defaults, key: secretgramScopedSettingsKey(accountPeerId: accountPeerId, key: key))
...
private func secretgramProjectActiveSettings(accountPeerId: Int64, state: GhostBaseSettingsState) {
    secretgramPersistChangedSettings(accountPeerId: accountPeerId, previous: nil, current: state)
}
...
struct GhostBaseSettingsState: Equatable {
    ...
    static func load(accountPeerId: Int64) -> GhostBaseSettingsState {
        return GhostBaseSettingsState(
            profileEnabled: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.profileEnabled, defaultValue: true),
            showIds: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.showIds, defaultValue: true),
            showDCs: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.showDCs, defaultValue: true),
            showRegistration: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.showRegistration, defaultValue: true),
            glassEnabled: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.glassEnabled, defaultValue: true),
            profileAvatarBlur: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.profileAvatarBlur, defaultValue: true),
            profileAnimatedBackground: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.profileAnimatedBackground, defaultValue: true),
            profileBlurTint: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.profileBlurTint, defaultValue: true),
            profileBlurReduced: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.profileBlurReduced, defaultValue: false),
            readMessages: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.readMessages, defaultValue: false),
            typingActions: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.typingActions, defaultValue: false),
            recordingActions: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.recordingActions, defaultValue: false),
            uploadingActions: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.uploadingActions, defaultValue: false),
            stickerActivity: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.stickerActivity, defaultValue: false),
            gameActivity: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.gameActivity, defaultValue: false),
            emojiActivity: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.emojiActivity, defaultValue: false),
            presence: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.presence, defaultValue: false),
            scheduledSend: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.scheduledSend, defaultValue: false),
            saveDeleted: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.saveDeleted, defaultValue: true),
            showDeleted: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.showDeleted, defaultValue: true),
            saveEditHistory: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.saveEditHistory, defaultValue: true),
            showEditHistory: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.showEditHistory, defaultValue: true),
            hideBlockedMessages: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.hideBlockedMessages, defaultValue: true),
            hideBlockedReactions: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.hideBlockedReactions, defaultValue: true),
            sendTextStyle: secretgramScopedString(accountPeerId: accountPeerId, key: 
                ghostBaseSendTextStyleKey,
                defaultValue: "normal"
            ),
            deletedPortableReplies: secretgramScopedBool(accountPeerId: accountPeerId, key: 
            ...
            preserveDeletedMedia: secretgramScopedBool(accountPeerId: accountPeerId, key: 
            ...
            showRamUnderClock: secretgramScopedBool(accountPeerId: accountPeerId, key: 
            ...
            messageSeconds: secretgramScopedBool(accountPeerId: accountPeerId, key: 
                GhostBaseKey.messageSeconds,
                defaultValue: false
            ),
            hideOwnPhone: secretgramScopedBool(accountPeerId: accountPeerId, key: 
                GhostBaseKey.hideOwnPhone,
                defaultValue: false
            ),
            protectedEnabled: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.protectedEnabled, defaultValue: true),
            protectedGalleryShare: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.protectedGalleryShare, defaultValue: true),
            protectedGallerySave: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.protectedGallerySave, defaultValue: true),
            protectedGalleryCopy: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.protectedGalleryCopy, defaultValue: true),
            chatSave: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.chatSave, defaultValue: true),
            chatCopy: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.chatCopy, defaultValue: true),
            chatForward: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.chatForward, defaultValue: true),
            allowScreenshots: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.allowScreenshots, defaultValue: true),
            allowScreenRecording: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.allowScreenRecording, defaultValue: true),
            oneTimeScreenshots: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.oneTimeScreenshots, defaultValue: false),
            oneTimeScreenRecording: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.oneTimeScreenRecording, defaultValue: false),
            oneTimeSave: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.oneTimeSave, defaultValue: false),
            storySave: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.storySave, defaultValue: false),
            localStarsEnabled: secretgramScopedBool(accountPeerId: accountPeerId, key: GhostBaseKey.localStarsEnabled, defaultValue: false),
            localStarsAmount: secretgramScopedString(accountPeerId: accountPeerId, key: GhostBaseKey.localStarsAmount, defaultValue: "0"),
            localStarsBaseAmount: secretgramScopedString(accountPeerId: accountPeerId, key: GhostBaseKey.localStarsBaseAmount, defaultValue: "0")
        )
    }

    func save(accountPeerId: Int64) {
        secretgramPersistChangedSettings(accountPeerId: accountPeerId, previous: nil, current: self)
    }
}


private func secretgramSettingsMenuIcon(
    _ name: String
) -> UIImage? {
    let background: UInt32

    switch name {
    case "SecretGram/Settings/Airplane":
        background = 0x53606A
...
private enum GhostBaseSettingsPage: Equatable {
    ...
    func localizedTitle(_ strings: SecretGramStrings) -> String {
        switch self {
        case .root:
            return strings.settingsTitle
        case .plugins:
            return strings.languageCode == "ru" ? "Плагины" : "Plugins"
        case .dataAndBackup:
...
// Shared visual contract used by every internal SecretGram destination.
private func SecretGramSettingsSectionHeaderItem(
...
private func SecretGramSettingsStatusItem(
    presentationData: ItemListPresentationData,
    text: String,
    sectionId: ItemListSectionId
) -> ListViewItem {
...
private let secretgramTelemetryEnabledKey = "secretgram.telemetry.anonymous.enabled"
private func secretgramTelemetryEnabled() -> Bool {
    let defaults = UserDefaults.standard
    return defaults.object(forKey: secretgramTelemetryEnabledKey) == nil ? true : defaults.bool(forKey: secretgramTelemetryEnabledKey)
}
private func secretgramSetTelemetryEnabled(_ enabled: Bool) {
    UserDefaults.standard.set(enabled, forKey: secretgramTelemetryEnabledKey)
    NotificationCenter.default.post(name: Notification.Name("SecretGramTelemetryPreferenceChanged"), object: nil)
}
...
private enum GhostBaseSettingsEntry: ItemListNodeEntry {
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        ...
        case let .header(_, text):
            return SecretGramSettingsSectionHeaderItem(presentationData: presentationData, text: text, sectionId: self.section)
        ...
        case let .telemetryToggle(_, _, title, value):
            return ItemListSwitchItem(
                ...
                style: .blocks,
                updated: { enabled in
                    secretgramSetTelemetryEnabled(enabled)
                }
            )
        ...
        case let .disclosure(_, _, title, iconName, page):
            return ItemListDisclosureItem(
                presentationData: presentationData,
                systemStyle: .glass,
                icon: secretgramSettingsMenuIcon(iconName),
                title: title,
                label: "",
                labelStyle: .text,
                sectionId: self.section,
        ...
        case let .selector(_, _, title, value):
            return ItemListDisclosureItem(
                ...
                attributedLabel: ghostBaseSendStyleAttributedText(
                    ...
                    value,
                    strings: presentationData.strings.secretgram
                ),
        ...
        case let .stylePreview(_, _, value):
            let line = NSMutableAttributedString(
                string: presentationData.strings.secretgram.sendStyleExamplePrefix,
                attributes: [
            ...
            line.append(
                ghostBaseSendStyleAttributedText(
                    style: value,
                    text: presentationData.strings.secretgram.sendStyleExampleBody,
        ...
        case let .researchInfo(_, _, text):
            return SecretGramSettingsStatusItem(presentationData: presentationData, text: text, sectionId: self.section)
...
private let ghostBaseHiddenGiftsPrefix =
    "secretgram.Research.HiddenGifts."
...
private let ghostBaseBotCapabilityPrefix =
    "secretgram.Research.BotCapability."
...
private let ghostBaseBotDifferencePrefix =
    "secretgram.Research.BotDifference."
...
private let ghostBaseProfileIntelPrefix =
    "secretgram.Research.ProfileIntel1."
...
private let ghostBaseProfileIntel2Prefix =
    "secretgram.Research.ProfileIntel2."
...
private enum SecretGramAboutChannelState: Equatable {
    case loading
    case available(peer: EnginePeer, preview: String)
    case unavailable
}

private func secretgramAboutChannelState(
    context: AccountContext,
    enabled: Bool,
    username: String
) -> Signal<SecretGramAboutChannelState, NoError> {
    guard enabled else {
        return .single(.unavailable)
    }

    return context.engine.peers.resolvePeerByName(name: username, referrer: nil)
    |> mapToSignal { result -> Signal<SecretGramAboutChannelState, NoError> in
        switch result {
        case .progress:
        ...
        case let .result(peer):
            ...
            |> map { viewData, _ -> SecretGramAboutChannelState in
...
private let secretgramSettingsDidImportNotification = Notification.Name("SecretGramSettingsDidImport")

func secretgramNotifySettingsImported(accountPeerId: Int64) {
    NotificationCenter.default.post(
        name: secretgramSettingsDidImportNotification,
        object: nil,
        userInfo: ["accountPeerId": NSNumber(value: accountPeerId)]
    )
}

private func secretgramSettingsImportRefreshSignal(
    accountPeerId: Int64,
    ...
    return Signal { subscriber in
        ...
        let observer = NotificationCenter.default.addObserver(
            forName: secretgramSettingsDidImportNotification,
            object: nil,
...
private func ghostBaseSettingsEntries(
    ...
    strings: SecretGramStrings,
    aboutChannelState: SecretGramAboutChannelState,
    aboutCommunityState: SecretGramAboutChannelState
    ...
    if page == .root {
        ...
        return [
            ...
            .disclosure(0, 1, strings.basicFunctions, "SecretGram/Settings/Airplane", .home),
            ...
            .disclosure(0, 10, strings.languageCode == "ru" ? "Плагины" : "Plugins", "Chat/Context Menu/FormatCode", .plugins)
        ]
    }
    ...
    if page == .about {
        return [
            ...
            .aboutValue(1, 1, strings.secretgramVersion, SecretGramReleaseIdentity.displayVersion),
            .aboutValue(1, 2, strings.build, SecretGramReleaseIdentity.build),
            .aboutValue(1, 3, strings.telegramBase, SecretGramReleaseIdentity.telegramBase),
            .header(2, strings.privacy),
            .info(2, strings.languageCode == "ru" ? "SecretGram не отправляет аналитику разработчику исходного приложения." : "SecretGram does not send analytics to the upstream developer.")
        ]
    }
    ...
    let ghostBasePushPrefix = "secretgram.V10E.Push."
...
"""))
    ...
    let ghostBaseNsePrefix = "secretgram.V10D.NSE."
...
"""))
    ...
    let ghostBaseCorePrefixV10C = "secretgram.V10C.Core."
...
"""))
    ...
    let ghostBaseRawPrefixV10F = "secretgram.V10F.Raw."
    let ghostBaseRawDefaultsV10F = UserDefaults.standard

    entries.append(.info(debug, """
v1.0Q Raw Delete Mapping:
QRawDeleteEvents: \(UserDefaults.standard.integer(forKey: "secretgram.V10Q.RawDeleteEvents"))
QLastRawDeleteSource: \(UserDefaults.standard.string(forKey: "secretgram.V10Q.LastRawDeleteSource") ?? "none")
QLastRawDeleteIdCount: \(UserDefaults.standard.integer(forKey: "secretgram.V10Q.LastRawDeleteIdCount"))
QLastMappedDeleteIdCount: \(UserDefaults.standard.integer(forKey: "secretgram.V10Q.LastMappedDeleteIdCount"))
QLastRawDeleteIds: \(UserDefaults.standard.string(forKey: "secretgram.V10Q.LastRawDeleteIds") ?? "none")
QLastMappedDeleteIds: \(UserDefaults.standard.string(forKey: "secretgram.V10Q.LastMappedDeleteIds") ?? "none")
QVerdict: \(UserDefaults.standard.string(forKey: "secretgram.V10Q.Verdict") ?? "none")

SH2 Standalone Share Scheduled:
SH2StandaloneScheduledIntercept: \(UserDefaults.standard.integer(forKey: "secretgram.SH2.StandaloneScheduledIntercept.Count"))
SH2LastStandalonePeerId: \(UserDefaults.standard.string(forKey: "secretgram.SH2.LastStandalonePeerId") ?? "none")
SH2LastStandaloneScheduleTime: \(UserDefaults.standard.integer(forKey: "secretgram.SH2.LastStandaloneScheduleTime"))

OT2 ViewOnce Visual Keep:
OT2ViewOnceVisibleKeep: \(UserDefaults.standard.integer(forKey: "secretgram.OT2.ViewOnceVisibleKeep.Count"))
OT2LastViewOnceVisibleId: \(UserDefaults.standard.string(forKey: "secretgram.OT2.LastViewOnceVisibleId") ?? "none")

v1.0P Pre-delete Shadow Trace:
PDeleteEvents: \(UserDefaults.standard.integer(forKey: "secretgram.V10P.DeleteEvents"))
PLastDeleteSource: \(UserDefaults.standard.string(forKey: "secretgram.V10P.LastDeleteSource") ?? "none")
PLastDeleteIdCount: \(UserDefaults.standard.integer(forKey: "secretgram.V10P.LastDeleteIdCount"))
PPreDeleteMessageHits: \(UserDefaults.standard.integer(forKey: "secretgram.V10P.PreDeleteMessageHits"))
PPreDeleteTextHits: \(UserDefaults.standard.integer(forKey: "secretgram.V10P.PreDeleteTextHits"))
PLastPreDeleteId: \(UserDefaults.standard.string(forKey: "secretgram.V10P.LastPreDeleteId") ?? "none")
PLastPreDeletePeer: \(UserDefaults.standard.string(forKey: "secretgram.V10P.LastPreDeletePeer") ?? "none")
PLastPreDeleteTextLength: \(UserDefaults.standard.integer(forKey: "secretgram.V10P.LastPreDeleteTextLength"))
PLastPreDeleteText: \(UserDefaults.standard.string(forKey: "secretgram.V10P.LastPreDeleteText") ?? "none")
PVerdict: \(UserDefaults.standard.string(forKey: "secretgram.V10P.Verdict") ?? "none")

SH1 Share Scheduled Send:
SH1ShareScheduledIntercept: \(UserDefaults.standard.integer(forKey: "secretgram.SH1.ShareScheduledIntercept.Count"))
SH1LastSharePeerId: \(UserDefaults.standard.string(forKey: "secretgram.SH1.LastSharePeerId") ?? "none")
SH1LastShareMessageCount: \(UserDefaults.standard.integer(forKey: "secretgram.SH1.LastShareMessageCount"))
SH1LastShareScheduleTime: \(UserDefaults.standard.integer(forKey: "secretgram.SH1.LastShareScheduleTime"))

OT1 Timer Media Local Keep:
OT1OutgoingKeepBlocked: \(UserDefaults.standard.integer(forKey: "secretgram.OT1.OutgoingKeepBlocked.Count"))
OT1AutoremoveKeepBlocked: \(UserDefaults.standard.integer(forKey: "secretgram.OT1.AutoremoveKeepBlocked.Count"))
OT1OutgoingKeepPath: \(UserDefaults.standard.string(forKey: "secretgram.OT1.OutgoingKeepPath") ?? "none")

v1.0O Persistent SourcePeer Candidate:
OSourcePeerPersistedRaw: \(UserDefaults.standard.object(forKey: "secretgram.V10O.Persistent.SourcePeerIdRaw") as? Int64 ?? 0)
OSourcePeerPersistedId: \(UserDefaults.standard.string(forKey: "secretgram.V10O.Persistent.SourcePeerId") ?? "none")
OSourcePeerPersistedSource: \(UserDefaults.standard.string(forKey: "secretgram.V10O.Persistent.SourcePeerSource") ?? "none")
...
READ3 Runtime Probe:
R3UIPeerIdRaw: \(UserDefaults.standard.object(forKey: "secretgram.READ3.UI.PeerIdRaw") as? Int64 ?? 0)
R3UIMessageId: \(UserDefaults.standard.object(forKey: "secretgram.READ3.UI.MessageId") as? Int32 ?? 0)
R3UIPeerType: \(UserDefaults.standard.string(forKey: "secretgram.READ3.UI.PeerType") ?? "none")
R3UIParticipantCount: \(UserDefaults.standard.object(forKey: "secretgram.READ3.UI.ParticipantCount") as? Int ?? -2)
R3UIThreshold: \(UserDefaults.standard.object(forKey: "secretgram.READ3.UI.Threshold") as? Int ?? -1)
R3UIAction: \(UserDefaults.standard.string(forKey: "secretgram.READ3.UI.Action") ?? "none")
R3LastPeerIdRaw: \(UserDefaults.standard.object(forKey: "secretgram.READ3.LastPeerIdRaw") as? Int64 ?? 0)
R3LastMessageId: \(UserDefaults.standard.object(forKey: "secretgram.READ3.LastMessageId") as? Int32 ?? 0)
R3LastApi: \(UserDefaults.standard.string(forKey: "secretgram.READ3.LastApi") ?? "none")
R3LastResponse: \(UserDefaults.standard.string(forKey: "secretgram.READ3.LastResponse") ?? "none")
R3ForcedApiRawCount: \(UserDefaults.standard.object(forKey: "secretgram.READ3.ForcedApiRawCount") as? Int ?? -1)
R3ForcedCount: \(UserDefaults.standard.object(forKey: "secretgram.READ3.ForcedCount") as? Int ?? -1)
R3FirstUserId: \(UserDefaults.standard.object(forKey: "secretgram.READ3.FirstUserId") as? Int64 ?? 0)
R3FirstReadDate: \(UserDefaults.standard.object(forKey: "secretgram.READ3.FirstReadDate") as? Int32 ?? 0)
R3LastErrorRaw: \(UserDefaults.standard.string(forKey: "secretgram.READ3.LastErrorRaw") ?? "none")
R3FinalVerdict: \(UserDefaults.standard.string(forKey: "secretgram.READ3.FinalVerdict") ?? "none")
...
"""))
    ...
    let ghostBaseCorePrefix = "secretgram.V10B.Core."
...
public func ghostBaseSettingsController(
    ...
    let rawPage = UserDefaults.standard.string(
        forKey: "secretgram.Settings.InitialPage"
    ) ?? "home"
...
private func ghostBaseSendStylePageController(
    ...
    |> map { presentationData, value
        ...
        let controllerState = ItemListControllerState(
            ...
            title: .text(presentationData.strings.secretgram.sendStyle),
...
private func ghostBaseSettingsPageController(
    ...
    secretgramProjectActiveSettings(accountPeerId: context.account.peerId.toInt64(), state: initialState)
    let statePromise = ValuePromise(initialState, ignoreRepeated: false)
    let stateValue = Atomic(value: initialState)
    let accountPeerId = context.account.peerId.toInt64()
    let secretgramImportRefreshSignal = secretgramSettingsImportRefreshSignal(
    ...
    let secretgramImportRefreshStateSignal = combineLatest(
        statePromise.get(),
        secretgramImportRefreshSignal
    )
    ...
    let updateState: ((GhostBaseSettingsState) -> GhostBaseSettingsState) -> Void = { f in
        let updated = stateValue.modify { current in
            ...
            secretgramPersistChangedSettings(
            ...
            if ghostBaseVisualSettingsChanged {
                NotificationCenter.default.post(
                    name: Notification.Name(
                        "secretgram.ProfileVisualSettingsDidChange.V11M"
                    ),
                    object: nil
                )
            }
    ...
    let aboutChannelSignal: Signal<SecretGramAboutChannelState, NoError> = .single(.unavailable)
    let aboutCommunitySignal: Signal<SecretGramAboutChannelState, NoError> = .single(.unavailable)
    ...
    let arguments = GhostBaseSettingsArguments(
        ...
        runResearchAction: { action in
            ...
            case "copyExtensionDiagnostics":
                UIPasteboard.general.string = BuildConfig.secretgramExtensionDiagnosticsReport()
        ...
        if selectedPage == .plugins {
            pushController?(secretgramPluginsController(context: context))
        } else if selectedPage == .dataAndBackup {
            pushController?(secretgramDataAndBackupController(context: context))
        } else if selectedPage == .stars {
            pushController?(secretgramStarsEditorController(context: context))
        } else {
            pushController?(
    ...
    let signal = combineLatest(
        context.sharedContext.presentationData,
        secretgramImportRefreshStateSignal,
        aboutChannelSignal,
    ...
    |> map { presentationData, state, aboutChannelState, aboutCommunityState
        ...
        let controllerState = ItemListControllerState(
            ...
            title: .text(page.localizedTitle(presentationData.strings.secretgram)),
            leftNavigationButton: nil,
            rightNavigationButton: nil,
