...
private let ghostBaseSafeLoginEnabledKey = "secretgram.SafeLogin.GhostModeEnabled"

private let ghostBaseSafeLoginGhostModeKeys: [String] = [
    "secretgram.GhostMode.ReadMessages",
    "secretgram.GhostMode.TypingActions",
    "secretgram.GhostMode.RecordingActions",
    "secretgram.GhostMode.UploadingActions",
    "secretgram.GhostMode.StickerActivity",
    "secretgram.GhostMode.GameActivity",
    "secretgram.GhostMode.EmojiActivity",
...
private func ghostBaseSafeLoginButtonTitle(_ enabled: Bool, strings: PresentationStrings) -> String {
    return strings.secretgram.authGhostModeStatus(enabled: enabled)
}
final class AuthorizationSequencePhoneEntryControllerNode: ASDisplayNode {
    ...
    init(sharedContext: SharedAccountContext, account: UnauthorizedAccount?, strings: PresentationStrings, theme: PresentationTheme, debugAction: @escaping () -> Void, hasOtherAccounts: Bool) {
        ...
        self.ghostBaseBotLoginNode.setAttributedTitle(
            NSAttributedString(
                string: strings.secretgram.botLoginButton,
        ...
        self.ghostBaseBotLoginNode.accessibilityLabel = strings.secretgram.botLoginAccessibility
        ...
        self.ghostBaseSafeLoginInfoNode.attributedText = NSAttributedString(string: strings.secretgram.authGhostModeHint, font: Font.regular(13.0), textColor: self.theme.list.itemPrimaryTextColor.withAlphaComponent(0.65))
    ...
    func containerLayoutUpdated(_ layout: ContainerViewLayout, navigationBarHeight: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        let secretgramKeyboardVisible = (layout.inputHeight ?? 0.0) > 0.0
        ...
        if self.hasOtherAccounts && !secretgramKeyboardVisible {
        ...
        self.ghostBaseBotLoginNode.isHidden = secretgramKeyboardVisible
        transition.updateFrame(
        ...
        self.ghostBaseSafeLoginNode.isHidden = secretgramKeyboardVisible
        self.ghostBaseSafeLoginInfoNode.isHidden = secretgramKeyboardVisible

        self.animationNode.updateLayout(size: animationSize)

        let secretgramOfficialAuthorizationBottomY = layout.size.height - insets.bottom - additionalBottomInset
        let secretgramAuthorizationBottomY: CGFloat
        if secretgramKeyboardVisible && layout.size.width > 320.0 {
            ...
            secretgramAuthorizationBottomY = min(
                secretgramOfficialAuthorizationBottomY,
                ghostBaseSafeLoginInfoFrame.minY - 10.0
            )
        } else {
            secretgramAuthorizationBottomY = secretgramOfficialAuthorizationBottomY
        }
        ...
        let _ = layoutAuthorizationItems(
            bounds: CGRect(
                ...
                size: CGSize(
                    ...
                    height: max(0.0, secretgramAuthorizationBottomY - insets.top)
                )
            ),
            items: items,
