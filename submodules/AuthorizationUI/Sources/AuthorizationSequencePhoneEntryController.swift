            ...
            controller.loginWithPassword = {
                ...
                self.ghostBaseBotAuthorizationDisposable.set((
                    ...
                    error: {
                        ...
                        switch error {
                        case .alreadyAdded:
                            text = self.presentationData.strings.secretgram.botAlreadyAdded

                        case .invalidToken:
                            controller.passwordIsInvalid()
                            text = self.presentationData.strings.secretgram.botInvalidToken

                        case .floodWait:
                            text = self.presentationData.strings.secretgram.botFloodWait
                        ...
                        case .signUpRequired:
                            text = self.presentationData.strings.secretgram.botSignUpRequired

                        case let .rpc(code):
                            text = self.presentationData.strings.secretgram.botRpcRejected(code)
                        ...
                        controller.present(
                            textAlertController(
                                sharedContext: self.sharedContext,
                                title: self.presentationData.strings.secretgram.botLoginTitle,
                                text: text,
                                actions: [
                                    TextAlertAction(
                                        type: .defaultAction,
