...
def build(bazel, arguments):
    ...
    if arguments.unsigned:
        # Device IPA for downstream signing (AltStore / SideStore / ESign).
        # Never search for a developer identity or embed somebody else's profile.
        bazel_command_line.set_disable_provisioning_profiles()
        bazel_command_line.common_build_args += ['--features=disable_legacy_signing']
    if arguments.bazelArguments:
        bazel_command_line.common_build_args += shlex.split(arguments.bazelArguments)
...
if __name__ == '__main__':
    ...
    buildParser.add_argument(
        ...
        help='Build a device IPA without provisioning profiles or signing, for subsequent user signing.'
    )
    buildParser.add_argument(
        '--buildNumber',
        required=True,
