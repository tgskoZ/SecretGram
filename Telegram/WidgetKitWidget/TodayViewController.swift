...
BuildConfig.secretgramRecordExtensionDiagnostic(
    ...
    guard let appGroupUrl = maybeAppGroupUrl else {
        let message = BuildConfig.secretgramExtensionBoundarySummary(
    ...
    let classification = BuildConfig.secretgramExtensionContainerClassification(path: appGroupUrl.path)
...
@available(iOSApplicationExtension 14.0, iOS 14.0, *)
private struct SecretGramWidgetDiagnosticView: View {
    let message: String
    var body: some View {
        VStack(spacing: 8.0) {
    ...
    var body: some View {
        GeometryReader(content: { geometry in
            if case let .diagnostic(message) = data {
                SecretGramWidgetDiagnosticView(message: message)
            } else {
