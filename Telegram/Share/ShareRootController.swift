...
class ShareRootController: UIViewController {
    ...
    private func showSecretGramExtensionDiagnostic(_ message: String) {
        let label = UILabel()
...
BuildConfig.secretgramRecordExtensionDiagnostic(
    ...
    detail: "containerURL resolved"
            ...
            guard let appGroupUrl = maybeAppGroupUrl else {
                self.showSecretGramExtensionDiagnostic(
                    BuildConfig.secretgramExtensionBoundarySummary(
                        process: "Share", stage: "container", path: nil
                    )
                )
                return
            }
            let classification = BuildConfig.secretgramExtensionContainerClassification(
